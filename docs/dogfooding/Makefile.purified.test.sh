#!/bin/sh
# Test Suite for Makefile.purified
...
test_determinism() {
    printf "Testing determinism for Makefile.purified...\n"

    # Run make twice and capture output
    make -f "Makefile.purified" > /tmp/output1.txt 2>&1 || true
    make -f "Makefile.purified" > /tmp/output2.txt 2>&1 || true
...
test_idempotency() {
    printf "Testing idempotency for Makefile.purified...\n"

    # Run make three times
    make -f "Makefile.purified" > /dev/null 2>&1 || true
    make -f "Makefile.purified" > /dev/null 2>&1 || exit_code1=$?
    make -f "Makefile.purified" > /dev/null 2>&1 || exit_code2=$?
...
test_posix_compliance() {
    printf "Testing POSIX compliance for Makefile.purified...\n"

    # Check if Makefile can be parsed by POSIX make
    # (Most systems have GNU make, so we just verify it doesn't error)
    if make -f "Makefile.purified" --version > /dev/null 2>&1; then
