...
| step | measured |
|---|---|
| PR | #291, merged as merge commit `a359431e8f` (branch head `6b619fcb4e`) |
| gate on the merge commit | CI run 34459917312: `ci / test` success 13 min, `ci / lint` 3 min, `ci / coverage` 8 min, `gate` success |
| tag | `v7.0.3` on `a359431e8f`, annotated (`tag-703.txt`) |
| GitHub release | release.yml run 34461275597 success; https://github.com/paiml/bashrs/releases/tag/v7.0.3 published 2026-09-10T09:34:55Z |
| crates.io | `cargo publish -p bashrs --locked` → Published bashrs v7.0.3 (09:33:27Z); API `max_version=7.0.3` |
| install | `cargo install bashrs --version 7.0.3 --locked` into a clean root: `bashrs 7.0.3`; every issue reproducer clean of its code (#255 via the Makefile path: 7.0.2 SC2168 → 7.0.3 none) |
| corpus (branch head, debug build) | 17,942 entries, 84.4/100 (B); A 100.0 · B1 99.9 · B2 99.9 · B3 98.7 · C 0.0 · D 100.0 (17,940/17,942) · E 100.0 · F 99.6 · G 99.7 — identical to 7.0.2 |
| corpus (installed 7.0.3, `corpus summary`) | 17,942 entries, 84.4/100 B, 333 failing entries; one entry swapped against the 7.0.2 list (B-13581 out, B-14434 in); `corpus show` scores both 85.0 with B3 and D PASS on both versions — exec-timing variance in F/G, not a rule effect |
| `pmat comply check` with 7.0.3 on PATH | CB-400 bashrs on the repo's own scripts: 1,253 errors / **5,849 → 5,584 warnings**; the other nine failing checks unchanged (Version Currency, CB-081-F, CB-200, CB-1308, CB-1351, CB-1650, CB-1700/1701, CB-2100) |
| pmat tickets | PMAT-249 completed (16/22, 2 overridden: complexity, file-size); PMAT-248 completed (12/22, 5 overridden: manifest — the "missing" file is `book/book/searchindex-<hash>.js`, regenerated by the book gate — complexity, file-size, dead-code in the touched `linter/mod.rs`, variant-coverage in the touched `semantic.rs`; all against DEBT-GATES-20260910) |

DoD: merged green on `gate` ✓ · gate exists ✓ · mutation observed RED (every phase's test committed failing first; 7 + 4 RED commits) ✓ · `pv` contract in the same PR (`linter-lexer-context-v1`, L2, valid) ✓ · discrimination confirmed (each test carries a true-positive twin; the review flipped one twin that was not a true positive) ✓ · invalidated doc claims updated in the same PR (book SC1012/SC1109/SC2046 sections, 7.0.3 section, CHANGELOG) ✓ · I-3 `transcript-gate`: attempted=16 denied=0 running_peak=3 slots=3 ✓ · `status-lint`: 4 blocks, all with basis= ✓ · `kind-gate` on the branch: kind=code files=35 ✓.

Gaps, named: the agy lanes could execute nothing (their findings were re-measured here, none taken on faith); `pv_lane` for kani harnesses = specified, not run (L2 claim); `cargo clippy --all-targets` on the local toolchain stops at the pre-existing `items_after_test_module` in `linter/suppression.rs` (CI's lint job, which is the authority, passed); SC2276 does not see a pipe on a backslash-continued line (named in the CHANGELOG).

Estimates: `K̂=7 [U]` from `estimate.sh` (one pooled row); K declared 120 (`docs/audits/impl-estimates.jsonl:L1-L2`); actual **36 turns** (`k_measured=283` − 247 at `goal.sh set`).

```
[status] ticket=PMAT-248 phase=7/7 global=36/7(K=120) k_measured=283 sub=0/0 basis=docs/audits/impl-estimates.jsonl:L1-L2
         mode=direct trigger=- route=self w=100.00 basis=absent q=? gate=PASS slots=0/3 denied=0
         red=- filed=PMAT-249,PMAT-250 blocker=- next=none — v7.0.3 is on GitHub and crates.io; PMAT-248 and PMAT-249 completed
```
