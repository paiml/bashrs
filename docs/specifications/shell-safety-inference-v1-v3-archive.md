...
**Version**: 4.0.0
**Status**: v4 BENCHMARK-DRIVEN REARCHITECTURE (v1-v3 retrospective complete, ShellSafeBench defined, provability contracts specified, linter-vs-ML comparative evaluation designed)
...
**Blocked on**: v4 benchmark-driven model shipping first (§20).

### 11.5 Class Imbalance Mitigation — ADDRESSED in §20.7

v3 data has 93.5% safe / 6.5% unsafe — 14.5:1 imbalance. v4 addresses this with:
- Curated unsafe expansion to ~3,000 entries (§20.7.1)
- Focal loss (§20.7.2)
- Hard negative mining (§20.7.3)
...
| Training converges (binary) | `apr finetune --num-classes 2` | Loss < 0.693 (random baseline) | **SUPERSEDED by §20** |
| Eval accuracy | `apr eval --num-classes 2` | Accuracy > 90% | **SUPERSEDED by §20** |
| HuggingFace publication | `apr publish` | Model card + weights on HF Hub | **SUPERSEDED by §20** |
...
## 20. v4: Retrospective, ShellSafeBench, and Provable Classification (2026-03-05)

### 20.1 Honest Retrospective: Why We Don't Have a Usable Model Yet

After 19 spec sections spanning v1 through v3.3, the project has produced zero published
models and zero reproducible benchmarks. This section applies Five Whys to the program-level
failure, not just a single training run.

#### Five Whys: Program-Level Failure

| # | Why? | Finding |
|---|------|---------|
| 1 | Why no published model on HuggingFace? | v3 training produced only epoch-1 results (91.4% val accuracy) — never completed multi-epoch training or eval |
| 2 | Why wasn't training completed? | Each training attempt exposed infrastructure bugs (wgpu per-op round-trips, monitoring eprintln, class_weights not persisted, multi-GPU hang), causing scope pivots |
| 3 | Why so many infrastructure bugs? | The training pipeline (entrenar/realizador) was being built concurrently with training — no "infrastructure freeze" before training phase |
| 4 | Why no infrastructure freeze? | No acceptance criteria for "training-ready" existed — the definition of done kept expanding |
| 5 | Why no acceptance criteria? | **No benchmark or evaluation standard was defined before starting training** — we built a pipeline without defining what success looks like |

**Root cause**: The project inverted the correct order. We should have defined
ShellSafeBench (the evaluation standard) FIRST, then built only the minimal pipeline
needed to meet that standard. Instead, we built an increasingly complex pipeline
(multi-GPU, wgpu, auto-tune, monitoring TUI, data parallelism) without ever defining
"what number do we need to hit on what benchmark to ship?"

#### Training Run History (Honest Summary)

| Version | Date | Model | Classes | Result | Outcome |
|---------|------|-------|---------|--------|---------|
| v1 | 2026-02 | MLP encoder (custom) | 5 | 62.2% accuracy | Below majority baseline for useful deployment |
| v2 | 2026-02-27 | Qwen2.5-Coder-0.5B LoRA | 5 | 35.6% eval accuracy | Prediction collapse — worse than random for minority classes |
| v2.2 | 2026-02-27 | Same, bug fixes | 5 | 73.2% val, diverged | class_weights bug, loss divergence (4.42 → 16.26) |
| v3 | 2026-03-01 | Qwen3-4B NF4 QLoRA | 2 | 91.4% val (epoch 1 only) | Promising but incomplete — no multi-epoch, no held-out eval, no publish |

**Honest assessment**: v3 epoch-1 at 91.4% is promising but unverified. The majority-class
baseline for binary (93.5% safe) means a model that always predicts "safe" gets 93.5%. The
91.4% number may be BELOW the majority baseline on the test set. We need proper evaluation.

### 20.2 Chain-of-Thought: What Does "Provably Useful" Mean?

A model is **provably useful** for bash failure classification when ALL of the following hold:

1. **Beats the rule-based linter on a defined benchmark** — the ML model must do something
   the existing 14-rule linter cannot. If the linter alone achieves equivalent recall on
   unsafe scripts, the ML model adds no value.

2. **Performance is bounded with confidence intervals** — not just "91.4% accuracy" but
   "91.4% ± 1.8% at 95% CI with N=1794 held-out samples". Bootstrap CIs already exist
   in `ClassifyEvalReport` (§16).

3. **Failure modes are characterized** — the confusion matrix, calibration curve, and
   per-class F1 must be published. Users must know WHERE the model fails (which categories
   of scripts it misclassifies and with what confidence).

4. **Reproducible from source** — anyone with the bashrs corpus can retrain and get
   equivalent results (within statistical tolerance). Deterministic splits, seeded training,
   published hyperparameters.

5. **Compared to meaningful baselines** — not just "random" and "majority class" but also:
   - The bashrs rule-based linter (SEC001-008 + DET001-006)
   - A simple regex/keyword classifier (does `$RANDOM` appear? → unsafe)
   - ShellCheck (static analysis)

### 20.3 ShellSafeBench: A Shell Safety Evaluation Benchmark

ShellSafeBench is a standardized benchmark for evaluating shell script safety classifiers.
It provides a fixed evaluation protocol that any classifier (ML-based, rule-based, hybrid)
can be evaluated against.

#### 20.3.1 Benchmark Design

**Principle**: Separate the evaluation data from the training data. The benchmark is NOT the
corpus — it is an independently curated test set with ground-truth labels verified by humans.

| Component | Description |
|-----------|-------------|
| **SSB-Core** | 500 hand-labeled shell scripts covering 10 failure categories (see §20.3.2) |
| **SSB-Adversarial** | 200 scripts designed to fool classifiers (obfuscation, unusual encoding, edge cases) |
| **SSB-Real-World** | 300 scripts from public GitHub repos / Stack Overflow with known CVEs or bugs |
| **Total** | 1,000 scripts with ground-truth labels |

**Scoring protocol**: Binary (safe=0, unsafe=1) primary metric, with secondary multi-label
annotations for failure category (for error analysis, not for the primary metric).

#### 20.3.2 Failure Categories (Ground-Truth Taxonomy)

These categories define WHAT failure the classifier should detect, enabling fine-grained
error analysis beyond binary safe/unsafe:

| Category | ID | Description | Example |
|----------|----|-------------|---------|
| Injection | F-INJ | Command injection via unquoted expansion | `eval "$user_input"` |
| Non-determinism | F-NDET | Output depends on runtime state | `echo $RANDOM`, `date +%s` |
| Non-idempotency | F-NIDEM | Unsafe to re-run | `mkdir /tmp/x` (no `-p`) |
| Path traversal | F-PATH | Unsanitized path manipulation | `cat "../../../etc/passwd"` |
| Race condition | F-RACE | TOCTOU or concurrent access | `test -f $f && rm $f` |
| Privilege escalation | F-PRIV | Unnecessary sudo/chmod 777 | `chmod 777 /etc/shadow` |
| Information leak | F-LEAK | Secrets in output/logs | `echo "password=$PASS"` |
| Resource exhaustion | F-RES | Fork bombs, unbounded loops | `:(){ :|:& };:` |
| Supply chain | F-SUPPLY | Curl-pipe-bash, unverified downloads | `curl $url | bash` |
| Encoding/quoting | F-QUOTE | Missing quotes causing word splitting | `rm -rf $DIR / important` |

Each script in the benchmark carries: `{id, script, binary_label, failure_categories[], source, verified_by}`.

#### 20.3.3 Metrics (Primary + Secondary)

**Primary metric** (for model comparison):

| Metric | Formula | Why |
|--------|---------|-----|
| **Unsafe Recall** | TP / (TP + FN) for class=unsafe | The model's #1 job: do not miss unsafe scripts |
| **Precision@90Recall** | Precision when recall threshold = 0.90 | At 90% unsafe recall, how many false alarms? |
| **MCC** | Matthews Correlation Coefficient | Best single metric for imbalanced binary classification |

**Secondary metrics** (for diagnostic analysis):

| Metric | Purpose |
|--------|---------|
| Per-category recall | Which failure types does the model miss? |
| ECE (Expected Calibration Error) | Are probability estimates trustworthy? |
| Adversarial robustness (SSB-Adversarial accuracy) | Does obfuscation fool it? |
| Inference latency (P50, P99) | Can it run in CI pipelines? |

#### 20.3.4 Baseline Implementations

Every ShellSafeBench evaluation MUST include these baselines for comparison:

| Baseline | Implementation | Expected Performance |
|----------|---------------|---------------------|
| **Majority class** | Always predicts "safe" | Recall=0%, Precision=undefined, MCC=0 |
| **Keyword heuristic** | Regex for `eval`, `$RANDOM`, `curl.*\|.*bash`, `chmod 777` | High precision, low recall (~40-60%) |
| **bashrs linter** | Run SEC001-008 + DET001-006 rules | High precision, moderate recall (rules-limited) |
| **ShellCheck** | `shellcheck -f json` → map findings to safe/unsafe | Depends on rule coverage |
| **SSC ML model** | The trained neural classifier | Must beat all above to justify deployment |

**Key question**: If the ML model does not beat `bashrs linter` + `keyword heuristic` combined,
the model adds no value and should not be shipped. The benchmark makes this comparison explicit.

### 20.4 Provability Contracts for SSC v4

"Provable" means the model ships with machine-verifiable contracts that gate publication.
If any contract fails, the model CANNOT be published to HuggingFace.

#### C-SSC-001: Unsafe Recall Floor

| Field | Value |
|-------|-------|
| **Precondition** | Model evaluated on SSB-Core (500 samples) |
| **Postcondition** | `unsafe_recall >= 0.85` (85% of unsafe scripts detected) |
| **Rationale** | Missing an unsafe script is the highest-cost failure mode |
| **Verification** | `apr eval --benchmark shellsafebench --assert-recall 0.85` |
| **Enforcement** | Publish pipeline exits with code 1 if violated |

#### C-SSC-002: MCC Above Baselines

| Field | Value |
|-------|-------|
| **Precondition** | Model and all baselines evaluated on identical SSB-Core set |
| **Postcondition** | `model_mcc > max(linter_mcc, keyword_mcc, shellcheck_mcc)` |
| **Rationale** | ML model must demonstrably outperform static analysis to justify its complexity and compute cost |
| **Verification** | `apr eval --benchmark shellsafebench --compare-baselines` |

#### C-SSC-003: Calibration Bound

| Field | Value |
|-------|-------|
| **Precondition** | Model produces probability estimates via softmax |
| **Postcondition** | `ECE < 0.10` (10% expected calibration error) |
| **Rationale** | Users need to trust confidence scores for risk-based triage |
| **Verification** | Included in `ClassifyEvalReport` (§16) |

#### C-SSC-004: Adversarial Robustness

| Field | Value |
|-------|-------|
| **Precondition** | Model evaluated on SSB-Adversarial (200 obfuscated/edge-case scripts) |
| **Postcondition** | `adversarial_accuracy >= 0.70` (within 20pp of SSB-Core accuracy) |
| **Rationale** | Real-world scripts use variable indirection, unusual quoting, encoded strings |
| **Verification** | `apr eval --benchmark shellsafebench --split adversarial` |

#### C-SSC-005: Latency Bound

| Field | Value |
|-------|-------|
| **Precondition** | Model loaded on CPU (no GPU required for inference) |
| **Postcondition** | P99 inference latency < 500ms per script |
| **Rationale** | Must be usable in CI/CD pipelines without GPU |
| **Verification** | `apr bench --task classify --data ssb-core.jsonl --p99-limit 500` |

#### C-SSC-006: Reproducibility

| Field | Value |
|-------|-------|
| **Precondition** | Two independent training runs with same seed, data, hyperparameters |
| **Postcondition** | MCC difference < 0.02 between runs |
| **Rationale** | Non-reproducible training means non-reproducible claims |
| **Verification** | `apr train --seed 42 && apr train --seed 42 → compare` |

### 20.5 Revised Architecture: Hybrid Linter + ML

The key insight from §20.2 is that ML should COMPLEMENT the rule-based linter, not replace
it. The v4 architecture formalizes this as a two-stage pipeline:

```text
Input: shell script
         │
    ┌────▼────┐
    │ Stage 1  │  bashrs linter (SEC001-008, DET001-006, IDEM001+)
    │ Rules    │  Deterministic, zero latency, zero false negatives for known patterns
    └────┬────┘
         │ scripts that pass all rules (no findings)
    ┌────▼────┐
    │ Stage 2  │  SSC neural classifier (Qwen3-4B or smaller)
    │ ML Model │  Catches patterns not covered by rules
    └────┬────┘
         │
    ┌────▼─────────────────────────────────────────┐
    │ Output: {safe, unsafe, confidence, findings[]} │
    │         findings from both stages combined     │
    └────────────────────────────────────────────────┘
```

**Why two stages?**

| Concern | Rules (Stage 1) | ML (Stage 2) | Hybrid |
|---------|----------------|--------------|--------|
| Known patterns (`eval`, `curl|bash`) | Perfect recall | May miss | Perfect |
| Novel/unknown patterns | Cannot detect | Can generalize | Can detect |
| Explainability | Rule ID + line number | Confidence score only | Best of both |
| Latency | <1ms | ~100-500ms | Conditional |
| False positives | Low (rules are precise) | Higher (learned patterns) | Rules filter first |

**Stage 2 is conditional**: If Stage 1 finds issues, the script is already classified as
unsafe with specific rule IDs. Stage 2 only runs on scripts that pass all rules — the
"looks clean but might not be" cases. This reduces ML inference cost to ~40% of scripts
(the 93.5% "safe" scripts in the corpus) and focuses the model on its hardest job.

### 20.6 Five Whys: Model Architecture Revisit

The v1-v3 training history suggests the current approach has structural issues beyond
infrastructure bugs:

| # | Why? | Finding |
|---|------|---------|
| 1 | Why did v3 binary only reach 91.4% (below majority 93.5%)? | 14.5:1 class imbalance — the model barely sees unsafe examples |
| 2 | Why is the corpus so imbalanced? | It is a transpiler test suite, not a curated safety dataset — most entries ARE safe |
| 3 | Why train on a transpiler test suite for safety classification? | The corpus was the only structured labeled data available |
| 4 | Why not curate a proper safety dataset? | Time was spent on pipeline infrastructure instead of data quality |
| 5 | Why prioritize infra over data? | **No benchmark existed to expose the data quality problem** |

**Decision**: Before any more training, build ShellSafeBench (§20.3) AND augment the
unsafe class with curated real-world examples.

### 20.7 Data Strategy: Fixing the 14.5:1 Imbalance

The corpus has 16,784 safe vs 1,158 unsafe entries. This is the wrong distribution for
training a safety classifier. Three concrete actions:

#### 20.7.1 Curated Unsafe Expansion (Target: 3,000 unsafe entries)

| Source | Entries | Description |
|--------|---------|-------------|
| CVE database (shell-related) | ~200 | Real exploits from NVD/CVE with shell vectors |
| ShellCheck rule violations | ~500 | Generate scripts that trigger each ShellCheck rule |
| OWASP command injection | ~300 | Canonical injection patterns from OWASP testing guide |
| GitHub security advisories | ~200 | Shell scripts from actual security incidents |
| Synthetic adversarial | ~800 | Adversarial generator with obfuscation (variable indirection, encoding tricks) |
| Existing corpus unsafe | 1,158 | Current unsafe entries |
| **Total target** | ~3,158 | ~18% of corpus (down from 6.5% → feasible for training) |

#### 20.7.2 Focal Loss Instead of Class Weights

v2/v3 used sqrt-inverse class weighting. This adjusts the loss globally but doesn't focus
the model on HARD examples. Focal loss (Lin et al., 2017) down-weights well-classified
examples and focuses on misclassified ones:

```
FL(p_t) = -alpha_t * (1 - p_t)^gamma * log(p_t)
```

With gamma=2.0, easy safe scripts (p_t ≈ 1.0) get near-zero loss contribution, while
misclassified unsafe scripts (p_t ≈ 0.0) get maximum gradient signal.

**Contract**: `C-HP-FOCAL-001: gamma ∈ [1.0, 3.0], alpha computed from class frequencies`

#### 20.7.3 Hard Negative Mining

After initial training, run inference on all training data. Scripts where the model is
confident but wrong (high loss) are "hard negatives". Augment the training set with
variants of these hard negatives for the next training epoch.

### 20.8 Model Size Decision: Qwen3-4B vs Qwen2.5-Coder-0.5B

v3 used Qwen3-4B (2560h, 36L, 5.9M trainable LoRA params). The question: is a 4B model
justified for binary classification of short scripts (median 73 chars)?

#### Five Whys: Do We Need 4B Parameters?

| # | Why? | Finding |
|---|------|---------|
| 1 | Why use 4B for 73-char inputs? | v2 on 0.5B diverged, so we scaled up |
| 2 | Why did 0.5B diverge? | class_weights bug (§17.2) + 5-class taxonomy mismatch (§18.1) |
| 3 | Would 0.5B work with bugs fixed + binary? | **Unknown — never tested** |
| 4 | Why never tested? | Jumped to 4B after diagnosing v2 without retrying on 0.5B |
| 5 | What does 4B give over 0.5B? | ~10x slower inference, ~4x more VRAM, unknown accuracy gain |

**Decision**: Train BOTH and compare on ShellSafeBench. The smaller model wins if
performance is within 2 MCC points, because:
- 0.5B enables CPU-only inference in CI/CD (<500ms P99)
- 0.5B can run in WASM (future browser deployment)
- 0.5B costs ~4x less compute per inference

#### Model Comparison Matrix (To Be Filled by Training)

| Metric | Qwen2.5-0.5B (binary, v4) | Qwen3-4B (binary, v4) | bashrs linter | Keyword heuristic |
|--------|---------------------------|----------------------|---------------|-------------------|
| SSB-Core MCC | TBD | TBD | TBD | TBD |
| Unsafe Recall | TBD | TBD | TBD | TBD |
| Precision@90Recall | TBD | TBD | TBD | TBD |
| SSB-Adversarial Acc | TBD | TBD | TBD | TBD |
| P99 Latency (CPU) | TBD (~50ms) | TBD (~400ms) | <1ms | <1ms |
| WASM compatible | Yes | No | Yes | Yes |

### 20.9 Implementation Plan (Ordered by Dependencies)

The plan is deliberately minimal. No multi-GPU, no wgpu, no auto-tune, no TUI.
Single GPU (RTX 4090), proven CUDA path, fixed hyperparameters from Appendix D contracts.

#### Phase 1: ShellSafeBench Construction (1 week)

| Task | Description | Deliverable |
|------|-------------|-------------|
| SSB-001 | Curate 500 SSB-Core scripts with ground-truth labels | `shellsafebench/core.jsonl` |
| SSB-002 | Curate 200 SSB-Adversarial scripts | `shellsafebench/adversarial.jsonl` |
| SSB-003 | Curate 300 SSB-Real-World scripts from GitHub/CVEs | `shellsafebench/real-world.jsonl` |
| SSB-004 | Implement `apr eval --benchmark shellsafebench` | CLI reads benchmark, computes metrics |
| SSB-005 | Run all 4 baselines on SSB | Baseline numbers for comparison table |

**Exit criterion**: All 4 baselines evaluated. We know the performance floor before ML.

#### Phase 2: Data Augmentation (3 days)

| Task | Description | Deliverable |
|------|-------------|-------------|
| DATA-001 | Expand unsafe class to ~3,000 entries (§20.7.1) | `corpus-v4.jsonl` |
| DATA-002 | Run `validate_export()` on augmented corpus | DataOps gate passes |
| DATA-003 | alimentar split (80/10/10 stratified, seed=42) | train/val/test parquet |

**Exit criterion**: Class imbalance < 6:1 (currently 14.5:1). Validation gate passes.

#### Phase 3: Training (2 days)

| Task | Description | Deliverable |
|------|-------------|-------------|
| TRAIN-001 | Qwen2.5-0.5B binary, CUDA, 3 epochs, HP contracts | Checkpoint + training log |
| TRAIN-002 | Qwen3-4B NF4 QLoRA binary, CUDA, 3 epochs, HP contracts | Checkpoint + training log |
| TRAIN-003 | `apr eval --benchmark shellsafebench` on both | Filled comparison matrix (§20.8) |

**Exit criterion**: Both models evaluated on SSB. Comparison matrix filled.

**Infrastructure constraint**: CUDA path on Lambda (RTX 4090) ONLY. No wgpu, no multi-GPU,
no intel machine. Use the path that worked in v3 epoch 1.

#### Phase 4: Contract Verification + Publication (1 day)

| Task | Description | Deliverable |
|------|-------------|-------------|
| PUB-001 | Verify C-SSC-001 through C-SSC-006 | All contracts pass or model does not ship |
| PUB-002 | Select winner (smaller model if within 2 MCC pts) | Final model selected |
| PUB-003 | `apr eval --generate-card` on winner | HuggingFace model card with SSB results |
| PUB-004 | `apr publish paiml/shell-safety-classifier` | Published on HuggingFace |
| PUB-005 | `bashrs classify --model paiml/shell-safety-classifier` integration test | End-to-end CLI works |

**Exit criterion**: Model published with benchmark results in model card. `bashrs classify` works.

### 20.10 What We Are NOT Doing in v4

Explicit scope exclusion to prevent the scope creep that killed v1-v3:

| NOT doing | Why |
|-----------|-----|
| Multi-GPU training | Single RTX 4090 is sufficient for 0.5B and 4B NF4 |
| wgpu backend training | CUDA path works, wgpu has unresolved AMD hang |
| Auto-tune hyperparameters | Use Appendix D contracts (peer-reviewed defaults) |
| TUI monitoring | `--verbose` stdout is sufficient for 3-epoch runs |
| 5-class taxonomy | Binary is the natural taxonomy (§18.1), revisit after binary ships |
| Multi-label classification | Ships after binary works |
| WASM inference | Ships after CPU inference works |
| Qwen3.5 upgrade | Ships after v4 model proves useful |

### 20.11 Success Criteria (Definition of Done)

v4 is **done** when ALL of the following are true:

- [ ] ShellSafeBench published (1,000 scripts, ground-truth labels, 4 baselines scored)
- [ ] At least one model passes C-SSC-001 (unsafe recall >= 85%)
- [ ] At least one model passes C-SSC-002 (MCC > best baseline)
- [ ] C-SSC-003 through C-SSC-006 pass
- [ ] Model published to `paiml/shell-safety-classifier` on HuggingFace
- [ ] Model card includes SSB results, baseline comparison, confidence intervals
- [ ] `bashrs classify --model paiml/shell-safety-classifier script.sh` works end-to-end
- [ ] Total elapsed time from Phase 1 start to publication: < 2 weeks

If no model meets C-SSC-001 + C-SSC-002 after Phase 3, the honest conclusion is that the
current corpus and model architecture are insufficient. In that case:
1. Publish ShellSafeBench as a standalone benchmark (value even without a model)
2. Document the failure modes for future work
3. Recommend the hybrid pipeline (§20.5) with linter-only until ML catches up

### 20.12 Verification Matrix (v4)

| Verification | Command | Expected Result | Status |
|-------------|---------|-----------------|--------|
| SSB-Core curated | `wc -l shellsafebench/core.jsonl` | 500 entries | PENDING |
| SSB-Adversarial curated | `wc -l shellsafebench/adversarial.jsonl` | 200 entries | PENDING |
| SSB-Real-World curated | `wc -l shellsafebench/real-world.jsonl` | 300 entries | PENDING |
| Baselines scored | `apr eval --benchmark shellsafebench --baselines` | 4 baselines with MCC/recall | PENDING |
| Unsafe expansion | Corpus class 1 count | >= 3,000 entries | PENDING |
| DataOps gate | `validate_export()` | PASS (imbalance < 6:1) | PENDING |
| 0.5B training | `apr finetune --model-size 0.5B --num-classes 2` | Completes 3 epochs | PENDING |
| 4B training | `apr finetune --model-size 4B --num-classes 2` | Completes 3 epochs | PENDING |
| C-SSC-001: Recall floor | `apr eval --assert-recall 0.85` | PASS | PENDING |
| C-SSC-002: MCC > baselines | `apr eval --compare-baselines` | PASS | PENDING |
| C-SSC-003: Calibration | ECE < 0.10 | PASS | PENDING |
| C-SSC-004: Adversarial | SSB-Adversarial accuracy >= 0.70 | PASS | PENDING |
| C-SSC-005: Latency | P99 < 500ms CPU | PASS | PENDING |
| C-SSC-006: Reproducibility | Two runs within 0.02 MCC | PASS | PENDING |
| HuggingFace published | `paiml/shell-safety-classifier` exists | Model + card live | PENDING |
| CLI integration | `bashrs classify --model ... script.sh` | Correct classification | PENDING |
