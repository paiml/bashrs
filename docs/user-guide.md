...
A comprehensive guide to using Rash—the Rust-to-Shell transpiler.
...
Shell scripts are everywhere—from `curl | sh` installers to CI/CD pipelines. But they're dangerous:
...
```rust
// No heap allocation
let vec = Vec::new();        // ❌ No dynamic allocation
let boxed = Box::new(42);    // ❌ No heap

// No complex types
struct Config { ... }        // ❌ No custom types
enum Status { ... }          // ❌ No enums

// No async/threads
async fn download() { ... }  // ❌ No async
thread::spawn(|| { ... });   // ❌ No threads

// No external crates
use reqwest::get;            // ❌ No dependencies
use serde_json::Value;       // ❌ Core features only

// No unsafe
unsafe { ... }               // ❌ No unsafe blocks
```
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("\n✓ Python {} installed successfully!", PYTHON_VERSION);
