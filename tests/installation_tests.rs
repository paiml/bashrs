...
fn test_generated_script_posix_compliance() {
    ...
    assert!(
        !script_content.contains("local "),
        "Generated script uses non-POSIX 'local'"
    );
    assert!(
        !script_content.contains("$'"),
        "Generated script uses non-POSIX $'...' syntax"
    );
    assert!(
        script_content.contains("#!/bin/sh"),
        "Missing POSIX shebang"
    );
    assert!(
        script_content.contains("set -euf"),
...
fn test_error_handling_in_generated_scripts() {
    ...
    assert!(
        script_content.contains("set -euf") || script_content.contains("set -e"),
