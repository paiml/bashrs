...
fn test_COV_EXEC_001_step_execution_result_fields() {
    let ok = StepExecutionResult {
        step_id: "s1".to_string(), success: true, exit_code: Some(0),
        stdout: "done".to_string(), stderr: String::new(),
        duration_ms: 1234, postcondition_results: vec![],
    };
    assert!(ok.success && ok.exit_code == Some(0));

    let fail = StepExecutionResult {
        step_id: "s2".to_string(), success: false, exit_code: Some(127),
        stdout: String::new(), stderr: "not found".to_string(),
        duration_ms: 50, postcondition_results: vec![],
    };
    assert!(!fail.success);

    let no_code = StepExecutionResult {
        step_id: "s3".to_string(), success: false, exit_code: None,
        stdout: String::new(), stderr: "signal".to_string(),
        duration_ms: 0, postcondition_results: vec![],
    };
...
fn test_COV_EXEC_002_postcondition_result() {
    let pass = PostconditionResult {
        check_type: "file_exists".to_string(), passed: true,
        details: "exists".to_string(),
    };
    assert!(pass.passed);

    let fail = PostconditionResult {
        check_type: "command_succeeds".to_string(), passed: false,
...
fn test_COV_EXEC_003_executor_config() {
    ...
    let custom = ExecutorConfig {
        dry_run: false, use_sudo: true,
        environment: {
            let mut e = HashMap::new();
            e.insert("K".to_string(), "V".to_string());
            e
        },
        working_dir: Some("/tmp".to_string()), timeout_secs: 300,
    };
...
fn test_COV_EXEC_004_executor_creation() {
    ...
    let cfg = ExecutorConfig { dry_run: true, use_sudo: true, ..Default::default() };
...
fn test_COV_EXEC_005_dry_run_script() {
    let ex = StepExecutor::with_config(ExecutorConfig { dry_run: true, ..Default::default() });
...
fn test_COV_EXEC_006_dry_run_apt_install() {
    ...
    let r2 = StepExecutor::new().execute_apt_install("empty", &[]).unwrap();
...
fn test_COV_EXEC_007_dry_run_file_write() {
    let ex = StepExecutor::with_config(ExecutorConfig { dry_run: true, ..Default::default() });
    let content = "[server]\nport = 8080";
    let r = ex.execute_file_write("w", "/etc/app.conf", content).unwrap();
    assert!(r.stdout.contains("[DRY-RUN]") && r.stdout.contains(&format!("{} bytes", content.len())));
...
content = "x"
"#;
    let r1 = StepExecutor::new().execute_step(&InstallerSpec::parse(toml1).unwrap().step[0]).unwrap();
...
path = "/tmp/t.txt"
"#;
    let r2 = StepExecutor::new().execute_step(&InstallerSpec::parse(toml2).unwrap().step[0]).unwrap();
...
"#;
    let r = StepExecutor::new().execute_step(&InstallerSpec::parse(toml).unwrap().step[0]).unwrap();
    assert!(!r.success && r.stderr.contains("Unknown action") && r.stderr.contains("deploy-to-mars"));
}
...
"#;
    let rp = StepExecutor::new().execute_step(&InstallerSpec::parse(toml_pass).unwrap().step[0]).unwrap();
    assert!(rp.success);
...
"#;
    let rf = StepExecutor::new().execute_step(&InstallerSpec::parse(toml_fail).unwrap().step[0]).unwrap();
...
fn test_COV_EXEC_017_check_file_exists_messages() {
    ...
    assert!(ex.check_file_exists("/no/such").details.contains("does not exist"));
}
...
fn test_COV_EXEC_018_executor_with_env_dry_run() {
    ...
    let ex = StepExecutor::with_config(ExecutorConfig {
        ...
        working_dir: Some("/tmp".to_string()), ..Default::default()
    });
    assert!(ex.execute_script("e", "sh", "echo $MY_VAR").unwrap().success);
}
