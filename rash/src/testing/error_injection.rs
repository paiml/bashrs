...
mod tests {
    ...
    fn test_parser_error_injection() {
        ...
        assert!(results.success_rate() > 90.0, "Parser error handling success rate too low: {:.1}%", results.success_rate());
    ...
    fn test_validation_error_injection() {
        ...
        assert!(results.success_rate() > 80.0, "Validation error handling success rate too low: {:.1}%", results.success_rate());
    }
    ...
    fn test_full_error_injection_suite() {
        ...
        assert!(results.total_injections > 50);
        assert!(results.success_rate() > 85.0, "Overall error injection success rate too low: {:.1}%", results.success_rate());
