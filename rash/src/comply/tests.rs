...
fn test_suppression_extract_multiple_rules() {
    ...
    assert_eq!(rules, Some(vec!["COMPLY-001".to_string(), "COMPLY-004".to_string()]));
}
...
fn test_suppression_parse_line_level() {
    ...
    assert_eq!(sup.line_level.get(&3), Some(&vec!["COMPLY-002".to_string()]));
}
...
fn test_suppression_file_level_only_first_10_lines() {
    ...
    let sup = runner::parse_suppressions(&content);
    assert!(sup.file_level.is_empty(), "Line 11 should not be file-level");
    assert_eq!(sup.line_level.get(&11), Some(&vec!["COMPLY-001".to_string()]));
}
...
fn test_suppression_file_level_comment_only() {
    ...
    let sup = runner::parse_suppressions(content);
    assert!(sup.file_level.is_empty(), "Inline code comment should not be file-level");
    assert_eq!(sup.line_level.get(&2), Some(&vec!["COMPLY-001".to_string()]));
}
...
fn test_suppression_apply_file_level() {
    ...
    assert!(suppressed.passed, "File-level suppression should clear violations");
...
fn test_suppression_apply_no_match() {
    ...
    let suppressed = runner::apply_suppressions(result, &sup);
    assert!(!suppressed.passed, "Different rule suppression should not affect this rule");
    assert_eq!(suppressed.violations.len(), 1);
}
...
fn test_suppression_apply_all_lines_suppressed() {
    ...
    let result = RuleResult {
        ...
        passed: false,
        violations: vec![
            Violation { rule: RuleId::Posix, line: Some(3), message: "v1".to_string() },
            Violation { rule: RuleId::Posix, line: Some(7), message: "v2".to_string() },
        ],
    };
    let suppressed = runner::apply_suppressions(result, &sup);
