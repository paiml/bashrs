...
fn suggest_fix(expected: &Token, found: Option<&Token>) -> Option<String> {
    ...
    match error {
        ParseError::UnexpectedToken { expected, found, line } => {
            ...
            let help = if expected.contains("then") {
                ...
                Some("every 'while'/'for'/'until' loop must be closed with 'done'".to_string())
            } else if expected.contains("esac") {
                Some("every 'case' must be closed with 'esac'".to_string())
            } else if expected.contains("in") {
                Some("'for' and 'case' require 'in': `for var in list` / `case $x in`".to_string())
            } else if expected.contains('}') {
                Some("unmatched '{' — did you forget the closing '}'?".to_string())
            } else if expected.contains(')') {
                Some("unmatched '(' — did you forget the closing ')'?".to_string())
            } else {
                None
        ...
        ParseError::LexerError(lex_err) => {
            let (line, col) = match lex_err {
                ...
                LexerError::UnterminatedString(l, c) => (Some(*l), Some(*c)),
