...
fn test_validate_ast_recursive_and_external_calls_allowed() {
    ...
    let ast = RestrictedAst {
        ...
        functions: vec![Function {
            ...
            params: vec![Parameter { name: "n".to_string(), param_type: Type::U32 }],
...
fn test_stmt_expr_valid_and_invalid() {
    assert!(Stmt::Expr(Expr::Variable("ok".to_string())).validate().is_ok());
    assert!(Stmt::Expr(Expr::Variable("".to_string())).validate().is_err());
}

#[test]
fn test_stmt_return_variants() {
    assert!(Stmt::Return(Some(Expr::Literal(Literal::U32(0)))).validate().is_ok());
    assert!(Stmt::Return(Some(Expr::Literal(Literal::Str("ok\0".to_string())))).validate().is_err());
...
fn test_stmt_if_validation_branches() {
    ...
    assert!(Stmt::If {
        ...
        then_block: vec![Stmt::Let { name: "".to_string(), value: Expr::Literal(Literal::U32(0)), declaration: true }],
        else_block: None,
    }.validate().is_err());
    ...
    assert!(Stmt::If {
        condition: Expr::Literal(Literal::Bool(true)),
        then_block: vec![],
        else_block: Some(vec![Stmt::Let { name: "".to_string(), value: Expr::Literal(Literal::U32(0)), declaration: true }]),
    }.validate().is_err());
}
...
fn test_stmt_match_validation_branches() {
    // Invalid scrutinee
    assert!(Stmt::Match { scrutinee: Expr::Variable("".to_string()), arms: vec![] }.validate().is_err());
    // Invalid pattern in arm
    assert!(Stmt::Match {
        scrutinee: Expr::Variable("x".to_string()),
        arms: vec![MatchArm { pattern: Pattern::Variable("".to_string()), guard: None, body: vec![] }],
    }.validate().is_err());
    // Invalid guard
    assert!(Stmt::Match {
        scrutinee: Expr::Variable("x".to_string()),
        arms: vec![MatchArm { pattern: Pattern::Wildcard, guard: Some(Expr::Variable("".to_string())), body: vec![] }],
    }.validate().is_err());
    // Invalid body
    assert!(Stmt::Match {
        scrutinee: Expr::Variable("x".to_string()),
        arms: vec![MatchArm {
            ...
            body: vec![Stmt::Let { name: "".to_string(), value: Expr::Literal(Literal::U32(0)), declaration: true }],
        }],
    }.validate().is_err());
}
...
fn test_stmt_for_validation_branches() {
    ...
    assert!(Stmt::For {
        ...
        body: vec![], max_iterations: Some(100),
    }.validate().is_ok());
    // No max_iterations
    assert!(Stmt::For {
        pattern: Pattern::Variable("i".to_string()),
        iter: Expr::Literal(Literal::U32(0)),
        body: vec![], max_iterations: None,
    }.validate().is_err());
    // Invalid pattern
    assert!(Stmt::For {
        pattern: Pattern::Variable("".to_string()),
        iter: Expr::Literal(Literal::U32(0)),
        body: vec![], max_iterations: Some(10),
    }.validate().is_err());
    // Invalid iter
    assert!(Stmt::For {
        pattern: Pattern::Variable("i".to_string()),
...
fn test_stmt_while_validation_branches() {
    assert!(Stmt::While {
        condition: Expr::Literal(Literal::Bool(true)),
        body: vec![Stmt::Break], max_iterations: Some(100),
    }.validate().is_ok());
    // No max_iterations
    assert!(Stmt::While {
        condition: Expr::Literal(Literal::Bool(true)),
...
fn test_stmt_collect_calls_variants() {
    // Let
    let mut calls = vec![];
    Stmt::Let { name: "x".to_string(), value: Expr::FunctionCall { name: "foo".to_string(), args: vec![] }, declaration: true }
        .collect_function_calls(&mut calls);
    assert_eq!(calls, vec!["foo"]);

    // Return(Some)
    let mut calls = vec![];
    Stmt::Return(Some(Expr::FunctionCall { name: "compute".to_string(), args: vec![] }))
        .collect_function_calls(&mut calls);
...
fn test_expr_wildcard_arms_validate_ok() {
    ...
    assert!(Expr::Try { expr: Box::new(Expr::Literal(Literal::U32(0))) }.validate().is_ok());
    assert!(Expr::Index {
...
fn test_nesting_depth_base_cases() {
    ...
    assert_eq!(Expr::FunctionCall { name: "f".to_string(), args: vec![] }.nesting_depth(), 1);
}
...
fn test_nesting_depth_method_call() {
    let expr = Expr::MethodCall {
        receiver: Box::new(Expr::MethodCall {
            ...
            method: "trim".to_string(), args: vec![],
        }),
...
fn test_expr_collect_calls_method_and_unary() {
    let mut calls = vec![];
    Expr::MethodCall {
        receiver: Box::new(Expr::FunctionCall { name: "get".to_string(), args: vec![] }),
        method: "do_thing".to_string(),
        args: vec![Expr::FunctionCall { name: "helper".to_string(), args: vec![] }],
    }.collect_function_calls(&mut calls);
    ...
    Expr::Unary {
        op: UnaryOp::Not,
        operand: Box::new(Expr::FunctionCall { name: "check".to_string(), args: vec![] }),
    }.collect_function_calls(&mut calls);
...
fn test_pattern_validation_edge_cases() {
    ...
    assert!(Pattern::Struct {
        name: "P".to_string(),
        fields: vec![("x".to_string(), Pattern::Literal(Literal::Str("n\0".to_string())))],
    }.validate().is_err());
    assert!(Pattern::Tuple(vec![Pattern::Wildcard, Pattern::Variable("".to_string())]).validate().is_err());
    assert!(Pattern::Range { start: Literal::U32(0), end: Literal::U32(100), inclusive: true }.validate().is_ok());
}

#[test]
fn test_pattern_binds_variable_range() {
    assert!(!Pattern::Range { start: Literal::U32(0), end: Literal::U32(10), inclusive: false }.binds_variable("x"));
}
...
fn test_type_nested_is_allowed() {
    ...
    assert!(Type::Result {
        ok_type: Box::new(Type::Option { inner_type: Box::new(Type::U32) }),
...
fn test_function_body_and_param_validation() {
    assert!(Function {
        name: "test".to_string(), params: vec![], return_type: Type::Void,
        body: vec![Stmt::Let { name: "".to_string(), value: Expr::Literal(Literal::U32(0)), declaration: true }],
    }.validate().is_err());
    assert!(Function {
        name: "test".to_string(),
        params: vec![Parameter { name: "$invalid".to_string(), param_type: Type::U32 }],
