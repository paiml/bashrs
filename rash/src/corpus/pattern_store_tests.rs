...
fn test_PSTORE_001_classify_all_pass() {
    let r = CorpusResult {
        transpiled: true, output_contains: true, output_exact: true,
        output_behavioral: true, lint_clean: true, cross_shell_agree: true,
        ..Default::default()
    };
...
fn test_PSTORE_002_classify_transpile_fail_gates() {
    let r = CorpusResult { transpiled: false, ..Default::default() };
...
fn test_PSTORE_003_classify_transpile_fail_ignores_true_flags() {
    let r = CorpusResult {
        transpiled: false, output_contains: true, output_exact: true,
        output_behavioral: true, lint_clean: true, cross_shell_agree: true,
        ..Default::default()
    };
...
fn test_PSTORE_004_classify_individual_signals() {
    // B1 only
    let b1 = CorpusResult { transpiled: true, output_contains: false,
        output_exact: true, output_behavioral: true, lint_clean: true,
        cross_shell_agree: true, ..Default::default() };
    assert!(classify_failure_signals(&b1).contains(&"B1_containment_fail".into()));

    // B2 only
    let b2 = CorpusResult { transpiled: true, output_contains: true,
        output_exact: false, output_behavioral: true, lint_clean: true,
        cross_shell_agree: true, ..Default::default() };
    assert_eq!(classify_failure_signals(&b2), vec!["B2_exact_fail"]);

    // B3 only
    let b3 = CorpusResult { transpiled: true, output_contains: true,
        output_exact: true, output_behavioral: false, lint_clean: true,
        cross_shell_agree: true, ..Default::default() };
    assert_eq!(classify_failure_signals(&b3), vec!["B3_behavioral_fail"]);

    // D only
    let d = CorpusResult { transpiled: true, output_contains: true,
        output_exact: true, output_behavioral: true, lint_clean: false,
        cross_shell_agree: true, ..Default::default() };
    assert_eq!(classify_failure_signals(&d), vec!["D_lint_fail"]);

    // G only
    let g = CorpusResult { transpiled: true, output_contains: true,
        output_exact: true, output_behavioral: true, lint_clean: true,
        cross_shell_agree: false, ..Default::default() };
...
fn test_PSTORE_005_classify_all_fail_after_transpile() {
    let r = CorpusResult {
        transpiled: true, output_contains: false, output_exact: false,
        output_behavioral: false, lint_clean: false, cross_shell_agree: false,
        ..Default::default()
    };
    let signals = classify_failure_signals(&r);
...
fn test_PSTORE_006_classify_combinations() {
    // B1 + B2
    let r1 = CorpusResult { transpiled: true, output_contains: false,
        output_exact: false, output_behavioral: true, lint_clean: true,
        cross_shell_agree: true, ..Default::default() };
    assert_eq!(classify_failure_signals(&r1).len(), 2);
    // D + G
    let r2 = CorpusResult { transpiled: true, output_contains: true,
        output_exact: true, output_behavioral: true, lint_clean: false,
        cross_shell_agree: false, ..Default::default() };
...
fn test_PSTORE_007_mine_patterns_all_pass() {
    ...
    reg.add(bash_entry("T-PS-007", r#"fn main() { println!("hello"); }"#, "echo"));
...
fn test_PSTORE_008_mine_patterns_with_failure() {
    let runner = CorpusRunner::new(Config::default());
    let mut reg = CorpusRegistry::new();
    reg.add(bash_entry("T-PS-008a", r#"fn main() { println!("hi"); }"#, "echo"));
...
fn test_PSTORE_009_mine_patterns_empty_registry() {
    let store = mine_patterns(&CorpusRegistry::new(), &CorpusRunner::new(Config::default()));
...
fn test_PSTORE_010_mine_patterns_all_failures() {
    ...
    let mut reg = CorpusRegistry::new();
    for i in 0..3 {
        reg.add(bash_entry(&format!("T-PS-010-{i}"), &format!("broken {i}!!!"), "x"));
    }
    let store = mine_patterns(&reg, &runner);
...
fn test_PSTORE_011_mine_patterns_mixed_formats() {
    ...
    reg.add(CorpusEntry::new("T-PS-011b", "m", "m", CorpusFormat::Makefile,
...
fn test_PSTORE_012_suggest_entry_not_found() {
    let store = PatternStore {
        patterns: vec![], total_entries: 0, total_failures: 0, version: "1.0.0".into(),
    };
    assert!(suggest_fixes("nonexistent", &CorpusRegistry::new(),
        &CorpusRunner::new(Config::default()), &store).is_empty());
}
...
fn test_PSTORE_013_suggest_passing_entry() {
    let runner = CorpusRunner::new(Config::default());
    let mut reg = CorpusRegistry::new();
    reg.add(bash_entry("T-PS-013", r#"fn main() { println!("hi"); }"#, "echo"));
    let store = PatternStore {
        patterns: vec![ShellFixPattern {
            error_signal: "B1_containment_fail".into(),
            causal_decision: "assignment_value:single_quote".into(),
            fix_type: "quoting_strategy".into(), confidence: 0.8,
            evidence_ids: vec!["B-999".into()],
        }],
        total_entries: 100, total_failures: 5, version: "1.0.0".into(),
    };
...
fn test_PSTORE_014_suggest_failing_entry() {
    ...
    let store = PatternStore {
        patterns: vec![ShellFixPattern {
            ...
            fix_type: "ir_node_handling".into(), confidence: 0.9,
            evidence_ids: vec!["B-999".into()],
        }],
        total_entries: 100, total_failures: 5, version: "1.0.0".into(),
    };
...
fn test_PSTORE_015_pattern_fields_and_serde() {
    let pattern = ShellFixPattern {
        ...
        fix_type: "string_handling".into(), confidence: 0.72,
...
fn test_PSTORE_016_pattern_multiple_evidence_serde() {
    let pattern = ShellFixPattern {
        ...
        fix_type: "expansion_strategy".into(), confidence: 0.65,
        evidence_ids: vec!["B-100".into(), "B-200".into(), "B-300".into(), "B-400".into()],
    };
    let json = serde_json::to_string(&pattern).unwrap();
...
fn test_PSTORE_017_store_sorting() {
    let mut store = PatternStore {
        patterns: vec![
            ShellFixPattern { error_signal: "B1".into(), causal_decision: "a:b".into(),
                fix_type: "a".into(), confidence: 0.5, evidence_ids: vec!["1".into()] },
            ShellFixPattern { error_signal: "D".into(), causal_decision: "c:d".into(),
                fix_type: "c".into(), confidence: 0.9, evidence_ids: vec!["2".into()] },
            ShellFixPattern { error_signal: "G".into(), causal_decision: "e:f".into(),
                fix_type: "e".into(), confidence: 0.7, evidence_ids: vec!["3".into()] },
        ],
        total_entries: 100, total_failures: 10, version: "1.0.0".into(),
    };
    store.patterns.sort_by(|a, b| b.confidence.partial_cmp(&a.confidence).unwrap());
...
fn test_PSTORE_018_store_serde_roundtrip() {
    let store = PatternStore {
        patterns: vec![
            ShellFixPattern { error_signal: "B3_behavioral_fail".into(),
                causal_decision: "assignment_value:single_quote".into(),
                fix_type: "quoting_strategy".into(), confidence: 0.85,
                evidence_ids: vec!["B-143".into()] },
            ShellFixPattern { error_signal: "D_lint_fail".into(),
                causal_decision: "string_emit:unquoted".into(),
                fix_type: "string_handling".into(), confidence: 0.72,
                evidence_ids: vec!["B-100".into(), "B-200".into()] },
        ],
        total_entries: 900, total_failures: 5, version: "1.0.0".into(),
    };
    let loaded: PatternStore = serde_json::from_str(
...
fn test_PSTORE_020_debug_and_clone() {
    let pattern = ShellFixPattern {
        ...
        fix_type: "ir_node_handling".into(), confidence: 0.75,
    ...
    let store = PatternStore {
        patterns: vec![pattern], total_entries: 10, total_failures: 1,
        version: "1.0.0".into(),
    };
...
fn test_PSTORE_021_store_version() {
    let store = PatternStore {
        patterns: vec![], total_entries: 50, total_failures: 3, version: "2.0.0".into(),
    };
