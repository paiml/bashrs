...
impl CorpusRegistry {
    ...
    // =========================================================================
    // Expansion Wave 23: Bash B-501..B-530 (Adversarial: nested control flow,
    //   complex arithmetic, edge-case string handling, multi-function programs)
    // =========================================================================

    fn load_expansion18_bash(&mut self) {
        let entries = vec![
            CorpusEntry::new(
                "B-501",
                "nested-while-break",
                "Nested while loop with conditional break",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let mut i = 0; let mut total = 0; while i < 100 { i += 1; if i % 7 == 0 { total += i; } if total > 50 { break; } } }"#,
                "while [ \"$i\" -lt 100 ]",
            ),
            CorpusEntry::new(
                "B-502",
                "nested-if-elif-chain",
                "Deep if-elif-else chain with 5 branches",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn classify(x: i32) -> i32 { if x < 0 { 0 } else if x < 10 { 1 } else if x < 100 { 2 } else if x < 1000 { 3 } else { 4 } } fn main() { let a = classify(-5); let b = classify(7); let c = classify(50); let d = classify(500); let e = classify(5000); }"#,
                "classify() {",
            ),
            CorpusEntry::new(
                "B-503",
                "iterative-even-odd-parity",
                "Iterative parity check: reduce n by 2 until base case",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn is_even(n: i32) -> bool { n % 2 == 0 } fn is_odd(n: i32) -> bool { n % 2 != 0 } fn parity_label(n: i32) -> i32 { if is_even(n) { 0 } else { 1 } } fn count_evens(max: i32) -> i32 { let mut count = 0; let mut i = 0; while i <= max { if is_even(i) { count += 1; } i += 1; } count } fn main() { let a = is_even(4); let b = is_odd(3); let c = parity_label(7); let d = count_evens(10); }"#,
                "is_even() {",
            ),
            CorpusEntry::new(
                "B-504",
                "multi-return-path",
                "Function with multiple early return conditions",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn validate(x: i32, min: i32, max: i32) -> i32 { if x < min { return -1; } if x > max { return -2; } if x == 0 { return -3; } x } fn main() { let a = validate(5, 1, 10); let b = validate(-1, 1, 10); let c = validate(100, 1, 10); let d = validate(0, 1, 10); }"#,
                "validate() {",
            ),
            CorpusEntry::new(
                "B-505",
                "accumulator-pattern",
                "Accumulator pattern: build result across loop iterations",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn sum_of_squares(n: i32) -> i32 { let mut total = 0; let mut i = 1; while i <= n { total += i * i; i += 1; } total } fn sum_of_cubes(n: i32) -> i32 { let mut total = 0; let mut i = 1; while i <= n { total += i * i * i; i += 1; } total } fn main() { let s2 = sum_of_squares(5); let s3 = sum_of_cubes(4); }"#,
                "sum_of_squares() {",
            ),
            CorpusEntry::new(
                "B-506",
                ...
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn collatz_steps(start: i32) -> i32 { let mut n = start; let mut steps = 0; while n != 1 { if n % 2 == 0 { n = n / 2; } else { n = 3 * n + 1; } steps += 1; } steps } fn main() { let s1 = collatz_steps(6); let s2 = collatz_steps(27); let s3 = collatz_steps(1); }"#,
                "collatz_steps() {",
            ),
            CorpusEntry::new(
                "B-507",
                "digit-sum-iterative",
                "Iterative digit sum and digital root calculation",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn digit_sum(n: i32) -> i32 { let mut num = n; if num < 0 { num = 0 - num; } let mut total = 0; while num > 0 { total += num % 10; num = num / 10; } total } fn digital_root(n: i32) -> i32 { let mut result = n; while result >= 10 { result = digit_sum(result); } result } fn digit_count(n: i32) -> i32 { let mut count = 0; let mut val = n; if val == 0 { return 1; } if val < 0 { val = 0 - val; } while val > 0 { val = val / 10; count += 1; } count } fn main() { let ds = digit_sum(12345); let dr = digital_root(493); let dc = digit_count(12345); }"#,
                "digit_sum() {",
            ),
            CorpusEntry::new(
                "B-508",
                "state-machine-traffic-light",
                "State machine: traffic light transitions",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn next_state(state: i32) -> i32 { if state == 0 { 1 } else if state == 1 { 2 } else { 0 } } fn state_duration(state: i32) -> i32 { if state == 0 { 30 } else if state == 1 { 5 } else { 25 } } fn total_cycle() -> i32 { state_duration(0) + state_duration(1) + state_duration(2) } fn main() { let s1 = next_state(0); let s2 = next_state(s1); let s3 = next_state(s2); let cycle = total_cycle(); }"#,
                "next_state() {",
            ),
            CorpusEntry::new(
                "B-509",
                ...
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn is_prime(n: i32) -> bool { if n < 2 { return false; } let mut i = 2; while i * i <= n { if n % i == 0 { return false; } i += 1; } true } fn count_primes(max: i32) -> i32 { let mut count = 0; let mut n = 2; while n <= max { if is_prime(n) { count += 1; } n += 1; } count } fn main() { let c1 = count_primes(20); let c2 = count_primes(50); }"#,
                "is_prime() {",
            ),
            CorpusEntry::new(
                "B-510",
                "matrix-multiply-2x2",
                "2x2 matrix multiply via flat variables",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn mat_mul_2x2(a11: i32, a12: i32, a21: i32, a22: i32, b11: i32, b12: i32, b21: i32, b22: i32) -> i32 { a11 * b11 + a12 * b21 } fn mat_trace(a11: i32, a22: i32) -> i32 { a11 + a22 } fn mat_det(a11: i32, a12: i32, a21: i32, a22: i32) -> i32 { a11 * a22 - a12 * a21 } fn main() { let c11 = mat_mul_2x2(1, 2, 3, 4, 5, 6, 7, 8); let tr = mat_trace(1, 4); let det = mat_det(1, 2, 3, 4); }"#,
                "mat_mul_2x2() {",
            ),
            CorpusEntry::new(
                "B-511",
                "roman-numeral-value",
                "Convert single Roman numeral character to value",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn roman_value(ch: i32) -> i32 { if ch == 73 { 1 } else if ch == 86 { 5 } else if ch == 88 { 10 } else if ch == 76 { 50 } else if ch == 67 { 100 } else if ch == 68 { 500 } else if ch == 77 { 1000 } else { 0 } } fn main() { let i = roman_value(73); let v = roman_value(86); let x = roman_value(88); let m = roman_value(77); }"#,
                "roman_value() {",
            ),
            CorpusEntry::new(
                "B-512",
                "temperature-converter",
                "Multi-scale temperature conversion (C/F/K)",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn c_to_f(c: i32) -> i32 { c * 9 / 5 + 32 } fn f_to_c(f: i32) -> i32 { (f - 32) * 5 / 9 } fn c_to_k(c: i32) -> i32 { c + 273 } fn k_to_c(k: i32) -> i32 { k - 273 } fn f_to_k(f: i32) -> i32 { c_to_k(f_to_c(f)) } fn main() { let f = c_to_f(100); let c = f_to_c(212); let k = c_to_k(0); let c2 = k_to_c(373); let k2 = f_to_k(32); }"#,
                "c_to_f() {",
            ),
            CorpusEntry::new(
                "B-513",
                ...
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn popcount(n: i32) -> i32 { let mut count = 0; let mut val = n; while val > 0 { count += val % 2; val = val / 2; } count } fn is_power_of_two(n: i32) -> bool { n > 0 && popcount(n) == 1 } fn highest_bit(n: i32) -> i32 { let mut bit = 0; let mut val = n; while val > 1 { val = val / 2; bit += 1; } bit } fn main() { let p1 = popcount(255); let pw = is_power_of_two(64); let hb = highest_bit(100); }"#,
                "popcount() {",
            ),
            CorpusEntry::new(
                "B-514",
                "interval-overlap",
                "Interval arithmetic: overlap detection and merge",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn overlaps(a_start: i32, a_end: i32, b_start: i32, b_end: i32) -> bool { a_start < b_end && b_start < a_end } fn merge_start(a_start: i32, b_start: i32) -> i32 { if a_start < b_start { a_start } else { b_start } } fn merge_end(a_end: i32, b_end: i32) -> i32 { if a_end > b_end { a_end } else { b_end } } fn interval_len(start: i32, end: i32) -> i32 { if end > start { end - start } else { 0 } } fn main() { let o = overlaps(1, 5, 3, 8); let ms = merge_start(1, 3); let me = merge_end(5, 8); let l = interval_len(1, 5); }"#,
                "overlaps() {",
            ),
            CorpusEntry::new(
                "B-515",
                "run-length-counter",
                "Run-length encoding: count consecutive equal values",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn rle_count(prev: i32, curr: i32, run: i32) -> i32 { if curr == prev { run + 1 } else { 1 } } fn rle_emit(run: i32) -> bool { run > 1 } fn rle_compress_ratio(original: i32, compressed: i32) -> i32 { if original == 0 { 0 } else { compressed * 100 / original } } fn main() { let r1 = rle_count(65, 65, 1); let r2 = rle_count(65, 66, 2); let emit = rle_emit(3); let ratio = rle_compress_ratio(100, 40); }"#,
                "rle_count() {",
            ),
            CorpusEntry::new(
                "B-516",
                ...
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn isqrt(n: i32) -> i32 { if n < 2 { return n; } let mut x = n; let mut y = (x + 1) / 2; while y < x { x = y; y = (x + n / x) / 2; } x } fn is_perfect_square(n: i32) -> bool { let s = isqrt(n); s * s == n } fn main() { let s1 = isqrt(16); let s2 = isqrt(27); let p1 = is_perfect_square(25); let p2 = is_perfect_square(26); }"#,
                "isqrt() {",
            ),
            CorpusEntry::new(
                "B-517",
                ...
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn gcd(a: i32, b: i32) -> i32 { let mut x = a; let mut y = b; while y != 0 { let t = y; y = x % y; x = t; } x } fn totient(n: i32) -> i32 { let mut count = 0; let mut i = 1; while i < n { if gcd(i, n) == 1 { count += 1; } i += 1; } count } fn main() { let t1 = totient(12); let t2 = totient(7); let t3 = totient(1); }"#,
                "totient() {",
            ),
            CorpusEntry::new(
                "B-518",
                "crc-checksum-simple",
                "Simple CRC-style checksum accumulator",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn crc_update(crc: i32, byte: i32) -> i32 { let mut c = crc; let mut b = byte; let mut i = 0; while i < 8 { if (c + b) % 2 != 0 { c = c / 2; c = c + 128; } else { c = c / 2; } b = b / 2; i += 1; } c } fn crc_finalize(crc: i32) -> i32 { crc % 256 } fn main() { let mut c = 0; c = crc_update(c, 72); c = crc_update(c, 101); let final_crc = crc_finalize(c); }"#,
                "crc_update() {",
            ),
            CorpusEntry::new(
                "B-519",
                "scheduler-round-robin",
                "Round-robin scheduler: next process selection",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn next_process(current: i32, total: i32) -> i32 { (current + 1) % total } fn time_slice(priority: i32, base_quantum: i32) -> i32 { base_quantum * priority } fn total_wait(processes: i32, quantum: i32) -> i32 { (processes - 1) * quantum } fn turnaround(burst: i32, wait: i32) -> i32 { burst + wait } fn main() { let np = next_process(3, 5); let ts = time_slice(2, 10); let tw = total_wait(4, 10); let ta = turnaround(20, tw); }"#,
                "next_process() {",
            ),
            CorpusEntry::new(
                "B-520",
                "base-converter",
                "Number base converter (decimal to arbitrary base)",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn digit_at(n: i32, base: i32, pos: i32) -> i32 { let mut val = n; let mut i = 0; while i < pos { val = val / base; i += 1; } val % base } fn num_digits(n: i32, base: i32) -> i32 { if n == 0 { return 1; } let mut count = 0; let mut val = n; while val > 0 { val = val / base; count += 1; } count } fn from_base(d2: i32, d1: i32, d0: i32, base: i32) -> i32 { d2 * base * base + d1 * base + d0 } fn main() { let d = digit_at(255, 16, 0); let nd = num_digits(255, 2); let val = from_base(1, 0, 0, 2); }"#,
                "digit_at() {",
            ),
            CorpusEntry::new(
                "B-521",
                "levenshtein-distance-simple",
                "Simplified edit distance: count mismatches at each position",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn hamming_distance(a1: i32, a2: i32, a3: i32, b1: i32, b2: i32, b3: i32) -> i32 { let mut d = 0; if a1 != b1 { d += 1; } if a2 != b2 { d += 1; } if a3 != b3 { d += 1; } d } fn similarity_pct(distance: i32, length: i32) -> i32 { if length == 0 { 100 } else { (length - distance) * 100 / length } } fn main() { let hd = hamming_distance(1, 2, 3, 1, 0, 3); let sim = similarity_pct(hd, 3); }"#,
                "hamming_distance() {",
            ),
            CorpusEntry::new(
                "B-522",
                ...
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn refill_tokens(current: i32, max_tokens: i32, elapsed: i32, rate: i32) -> i32 { let added = elapsed * rate; let total = current + added; if total > max_tokens { max_tokens } else { total } } fn try_consume(tokens: i32, cost: i32) -> i32 { if tokens >= cost { tokens - cost } else { -1 } } fn wait_time(tokens: i32, cost: i32, rate: i32) -> i32 { if tokens >= cost { 0 } else if rate == 0 { -1 } else { (cost - tokens + rate - 1) / rate } } fn main() { let t = refill_tokens(5, 10, 3, 2); let r = try_consume(t, 3); let w = wait_time(2, 5, 2); }"#,
                "refill_tokens() {",
            ),
            CorpusEntry::new(
                "B-523",
                ...
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn horner3(a3: i32, a2: i32, a1: i32, a0: i32, x: i32) -> i32 { ((a3 * x + a2) * x + a1) * x + a0 } fn poly_derivative_at(a3: i32, a2: i32, a1: i32, x: i32) -> i32 { 3 * a3 * x * x + 2 * a2 * x + a1 } fn main() { let val = horner3(1, -3, 2, 1, 2); let deriv = poly_derivative_at(1, -3, 2, 2); }"#,
                "horner3() {",
            ),
            CorpusEntry::new(
                "B-524",
                "retry-with-backoff",
                "Exponential backoff retry calculator",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn backoff_delay(attempt: i32, base_ms: i32, max_ms: i32) -> i32 { let mut delay = base_ms; let mut i = 0; while i < attempt { delay = delay * 2; if delay > max_ms { delay = max_ms; } i += 1; } delay } fn total_wait(attempts: i32, base_ms: i32, max_ms: i32) -> i32 { let mut total = 0; let mut i = 0; while i < attempts { total += backoff_delay(i, base_ms, max_ms); i += 1; } total } fn should_retry(attempt: i32, max: i32) -> bool { attempt < max } fn main() { let d = backoff_delay(3, 100, 5000); let t = total_wait(5, 100, 5000); let r = should_retry(3, 5); }"#,
                "backoff_delay() {",
            ),
            CorpusEntry::new(
                "B-525",
                "circular-buffer-ops",
                "Circular buffer with wrap-around index arithmetic",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn cb_write_idx(head: i32, count: i32, cap: i32) -> i32 { (head + count) % cap } fn cb_read_idx(head: i32, offset: i32, cap: i32) -> i32 { (head + offset) % cap } fn cb_is_full(count: i32, cap: i32) -> bool { count >= cap } fn cb_is_empty(count: i32) -> bool { count == 0 } fn cb_available(count: i32, cap: i32) -> i32 { cap - count } fn main() { let wi = cb_write_idx(3, 5, 8); let ri = cb_read_idx(3, 2, 8); let full = cb_is_full(8, 8); let avail = cb_available(5, 8); }"#,
                "cb_write_idx() {",
            ),
            CorpusEntry::new(
                "B-526",
                ...
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn adler32_update(a: i32, b: i32, byte: i32) -> i32 { let new_a = (a + byte) % 65521; new_a } fn adler32_update_b(a: i32, b: i32, byte: i32) -> i32 { let new_a = (a + byte) % 65521; (b + new_a) % 65521 } fn adler32_combine(a: i32, b: i32) -> i32 { b * 65536 + a } fn main() { let mut a = 1; let mut b = 0; let new_a = adler32_update(a, b, 72); let new_b = adler32_update_b(a, b, 72); let checksum = adler32_combine(new_a, new_b); }"#,
                "adler32_update() {",
            ),
            CorpusEntry::new(
                "B-527",
                "game-of-life-cell",
                "Conway's Game of Life: single cell next state",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn next_cell(alive: bool, neighbors: i32) -> bool { if alive { neighbors == 2 || neighbors == 3 } else { neighbors == 3 } } fn count_alive(n1: bool, n2: bool, n3: bool, n4: bool, n5: bool, n6: bool, n7: bool, n8: bool) -> i32 { let mut c = 0; if n1 { c += 1; } if n2 { c += 1; } if n3 { c += 1; } if n4 { c += 1; } if n5 { c += 1; } if n6 { c += 1; } if n7 { c += 1; } if n8 { c += 1; } c } fn main() { let n = count_alive(true, false, true, false, true, false, false, false); let live = next_cell(true, n); let dead = next_cell(false, 2); }"#,
                "next_cell() {",
            ),
            CorpusEntry::new(
                "B-528",
                "coordinate-distance",
                "2D coordinate geometry: distance and midpoint",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn abs_val(x: i32) -> i32 { if x < 0 { 0 - x } else { x } } fn manhattan_dist(x1: i32, y1: i32, x2: i32, y2: i32) -> i32 { let dx = x2 - x1; let dy = y2 - y1; abs_val(dx) + abs_val(dy) } fn midpoint_x(x1: i32, x2: i32) -> i32 { (x1 + x2) / 2 } fn midpoint_y(y1: i32, y2: i32) -> i32 { (y1 + y2) / 2 } fn in_bounds(x: i32, y: i32, w: i32, h: i32) -> bool { x >= 0 && x < w && y >= 0 && y < h } fn main() { let d = manhattan_dist(1, 2, 4, 6); let mx = midpoint_x(0, 10); let ib = in_bounds(5, 5, 10, 10); }"#,
                "abs_val() {",
            ),
            CorpusEntry::new(
                "B-529",
                "vm-instruction-dispatch",
                "Simple bytecode VM: instruction fetch-decode-execute cycle",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn decode_opcode(instruction: i32) -> i32 { instruction / 100 } fn decode_operand(instruction: i32) -> i32 { instruction % 100 } fn execute(opcode: i32, acc: i32, operand: i32) -> i32 { if opcode == 1 { acc + operand } else if opcode == 2 { acc - operand } else if opcode == 3 { acc * operand } else if opcode == 4 { if operand != 0 { acc / operand } else { acc } } else { acc } } fn main() { let inst = 105; let op = decode_opcode(inst); let arg = decode_operand(inst); let result = execute(op, 10, arg); }"#,
                "decode_opcode() {",
            ),
            CorpusEntry::new(
                "B-530",
                "milestone-530-multifunction",
                "Milestone 530: Complex multi-function program — mini calculator with operation chaining",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn add(a: i32, b: i32) -> i32 { a + b } fn sub(a: i32, b: i32) -> i32 { a - b } fn mul(a: i32, b: i32) -> i32 { a * b } fn safe_div(a: i32, b: i32) -> i32 { if b == 0 { 0 } else { a / b } } fn negate(a: i32) -> i32 { 0 - a } fn abs_val(a: i32) -> i32 { if a < 0 { 0 - a } else { a } } fn clamp(val: i32, lo: i32, hi: i32) -> i32 { if val < lo { lo } else if val > hi { hi } else { val } } fn chain_ops(x: i32) -> i32 { let a = add(x, 10); let b = mul(a, 3); let c = sub(b, 5); let d = safe_div(c, 2); clamp(d, 0, 100) } fn main() { let r1 = chain_ops(5); let r2 = chain_ops(0); let r3 = chain_ops(-10); let neg = negate(42); let ab = abs_val(-99); }"#,
                "add() {",
            ),
        ];
        self.entries.extend(entries);
    ...
    // =========================================================================
    // Expansion Wave 23: Makefile M-201..M-220 (Adversarial/Production:
    //   conditional logic, computed targets, pattern rules, complex pipelines)
    // =========================================================================

    fn load_expansion11_makefile(&mut self) {
        let entries = vec![
            CorpusEntry::new(
                "M-201",
                "makefile-cross-compile",
                "Cross-compilation matrix for multiple target triples",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let targets = "x86_64-unknown-linux-gnu aarch64-unknown-linux-gnu x86_64-apple-darwin"; phony_target("cross-all", &["cross-linux-amd64", "cross-linux-arm64", "cross-macos"], &["@echo All targets built"]); phony_target("cross-linux-amd64", &[], &["cargo build --release --target x86_64-unknown-linux-gnu"]); phony_target("cross-linux-arm64", &[], &["cross build --release --target aarch64-unknown-linux-gnu"]); phony_target("cross-macos", &[], &["cargo build --release --target x86_64-apple-darwin"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: cross-all",
            ),
            CorpusEntry::new(
                "M-202",
                ...
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let proto_dir = "proto"; phony_target("proto-gen", &[], &["buf generate $(PROTO_DIR)"]); phony_target("proto-lint", &[], &["buf lint $(PROTO_DIR)"]); phony_target("proto-breaking", &[], &["buf breaking $(PROTO_DIR) --against .git"]); phony_target("proto-clean", &[], &["rm -rf gen/"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: proto-gen",
            ),
            CorpusEntry::new(
                "M-203",
                "makefile-nix-build",
                "Nix-based reproducible build pipeline",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("nix-build", &[], &["nix build .#default"]); phony_target("nix-develop", &[], &["nix develop"]); phony_target("nix-check", &[], &["nix flake check"]); phony_target("nix-update", &[], &["nix flake update"]); phony_target("nix-gc", &[], &["nix-collect-garbage -d"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: nix-build",
            ),
            CorpusEntry::new(
                "M-204",
                "makefile-fuzz-testing",
                "Fuzz testing pipeline with cargo-fuzz",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { let fuzz_target = "parse"; let max_time = "300"; phony_target("fuzz", &[], &["cargo +nightly fuzz run $(FUZZ_TARGET) -- -max_total_time=$(MAX_TIME)"]); phony_target("fuzz-list", &[], &["cargo +nightly fuzz list"]); phony_target("fuzz-corpus", &[], &["@ls -la fuzz/corpus/$(FUZZ_TARGET)/"]); phony_target("fuzz-cov", &[], &["cargo +nightly fuzz coverage $(FUZZ_TARGET)"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: fuzz",
            ),
            CorpusEntry::new(
                "M-205",
                "makefile-release-matrix",
                "Multi-platform release artifact matrix",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let version = "$(shell git describe --tags)"; phony_target("release-all", &["release-linux", "release-macos", "release-windows"], &["@echo Releases built"]); phony_target("release-linux", &[], &["cargo build --release --target x86_64-unknown-linux-musl", "tar czf dist/app-$(VERSION)-linux-amd64.tar.gz -C target/x86_64-unknown-linux-musl/release app"]); phony_target("release-macos", &[], &["cargo build --release --target x86_64-apple-darwin"]); phony_target("release-windows", &[], &["cargo build --release --target x86_64-pc-windows-gnu"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: release-all",
            ),
            CorpusEntry::new(
                "M-206",
                ...
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let chart = "charts/app"; let ns = "production"; phony_target("helm-lint", &[], &["helm lint $(CHART)"]); phony_target("helm-template", &[], &["helm template app $(CHART) -n $(NS)"]); phony_target("helm-install", &["helm-lint"], &["helm upgrade --install app $(CHART) -n $(NS) --create-namespace"]); phony_target("helm-uninstall", &[], &["helm uninstall app -n $(NS)"]); phony_target("helm-package", &["helm-lint"], &["helm package $(CHART)"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: helm-lint",
            ),
            CorpusEntry::new(
                "M-207",
                "makefile-sqlx-prepare",
                "SQLx offline mode preparation",
                CorpusFormat::Makefile,
                CorpusTier::Standard,
                r#"fn main() { let db_url = "sqlite://app.db"; phony_target("sqlx-prepare", &[], &["cargo sqlx prepare --database-url $(DB_URL)"]); phony_target("sqlx-check", &[], &["cargo sqlx prepare --check --database-url $(DB_URL)"]); phony_target("sqlx-migrate", &[], &["sqlx migrate run --database-url $(DB_URL)"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: sqlx-prepare",
            ),
            CorpusEntry::new(
                "M-208",
                "makefile-openapi-codegen",
                "OpenAPI spec generation from Rust code",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let spec_file = "openapi.json"; phony_target("openapi-gen", &[], &["cargo run -- --openapi > $(SPEC_FILE)"]); phony_target("openapi-validate", &["openapi-gen"], &["npx @redocly/cli lint $(SPEC_FILE)"]); phony_target("openapi-docs", &["openapi-gen"], &["npx @redocly/cli build-docs $(SPEC_FILE) -o docs/api.html"]); phony_target("openapi-client", &["openapi-gen"], &["npx openapi-generator-cli generate -i $(SPEC_FILE) -g typescript-fetch -o client/"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: openapi-gen",
            ),
            CorpusEntry::new(
                "M-209",
                "makefile-perf-profiling",
                "Performance profiling pipeline with flamegraph",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("profile", &["build-release"], &["perf record -g target/release/app", "perf script | inferno-collapse-perf | inferno-flamegraph > flamegraph.svg"]); phony_target("profile-heap", &[], &["heaptrack target/release/app"]); phony_target("build-release", &[], &["cargo build --release"]); phony_target("bench-record", &[], &["cargo bench -- --save-baseline current"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: profile",
            ),
            CorpusEntry::new(
                "M-210",
                "makefile-feature-flags",
                "Feature flag combinations for CI testing",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("test-all-features", &[], &["cargo test --all-features"]); phony_target("test-no-features", &[], &["cargo test --no-default-features"]); phony_target("test-feature-wasm", &[], &["cargo test --features wasm"]); phony_target("test-feature-simd", &[], &["cargo test --features simd"]); phony_target("test-feature-matrix", &["test-all-features", "test-no-features", "test-feature-wasm", "test-feature-simd"], &["@echo Feature matrix complete"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: test-all-features",
            ),
            CorpusEntry::new(
                "M-211",
                "makefile-sops-secrets",
                "SOPS encrypted secrets management",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let secrets = "secrets.enc.yaml"; phony_target("secrets-edit", &[], &["sops $(SECRETS)"]); phony_target("secrets-decrypt", &[], &["sops -d $(SECRETS) > secrets.yaml"]); phony_target("secrets-encrypt", &[], &["sops -e secrets.yaml > $(SECRETS)"]); phony_target("secrets-rotate", &[], &["sops --rotate -i $(SECRETS)"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: secrets-edit",
            ),
            CorpusEntry::new(
                "M-212",
                "makefile-cargo-deny",
                "License and vulnerability checking with cargo-deny",
                CorpusFormat::Makefile,
                CorpusTier::Standard,
                r#"fn main() { phony_target("deny-all", &["deny-licenses", "deny-advisories", "deny-bans"], &["@echo All deny checks passed"]); phony_target("deny-licenses", &[], &["cargo deny check licenses"]); phony_target("deny-advisories", &[], &["cargo deny check advisories"]); phony_target("deny-bans", &[], &["cargo deny check bans"]); phony_target("deny-sources", &[], &["cargo deny check sources"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: deny-all",
            ),
            CorpusEntry::new(
                "M-213",
                "makefile-docker-compose-dev",
                "Docker Compose development environment",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let compose = "docker-compose.dev.yml"; phony_target("dev-up", &[], &["docker compose -f $(COMPOSE) up -d"]); phony_target("dev-down", &[], &["docker compose -f $(COMPOSE) down -v"]); phony_target("dev-logs", &[], &["docker compose -f $(COMPOSE) logs -f"]); phony_target("dev-rebuild", &[], &["docker compose -f $(COMPOSE) up -d --build"]); phony_target("dev-ps", &[], &["docker compose -f $(COMPOSE) ps"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: dev-up",
            ),
            CorpusEntry::new(
                "M-214",
                "makefile-grpc-health",
                "gRPC health check and load testing",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { let addr = "localhost:50051"; phony_target("grpc-health", &[], &["grpcurl -plaintext $(ADDR) grpc.health.v1.Health/Check"]); phony_target("grpc-list", &[], &["grpcurl -plaintext $(ADDR) list"]); phony_target("grpc-bench", &[], &["ghz --insecure --call api.Service/Method -n 10000 -c 100 $(ADDR)"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: grpc-health",
            ),
            CorpusEntry::new(
                "M-215",
                "makefile-sbom-generate",
                "Software Bill of Materials generation",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let sbom_dir = "sbom"; phony_target("sbom", &[], &["cargo cyclonedx --format json --output-file $(SBOM_DIR)/sbom.json"]); phony_target("sbom-verify", &["sbom"], &["cyclonedx-cli validate --input-file $(SBOM_DIR)/sbom.json"]); phony_target("sbom-diff", &[], &["diff $(SBOM_DIR)/sbom.json $(SBOM_DIR)/sbom.prev.json"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: sbom",
            ),
            CorpusEntry::new(
                "M-216",
                "makefile-miri-test",
                "Miri undefined behavior testing",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("miri", &[], &["cargo +nightly miri test"]); phony_target("miri-setup", &[], &["rustup +nightly component add miri"]); phony_target("miri-clean", &[], &["cargo +nightly miri clean"]); phony_target("unsafe-audit", &[], &["cargo geiger"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: miri",
            ),
            CorpusEntry::new(
                "M-217",
                "makefile-crate-publish",
                "Crate publishing pipeline with dry-run",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let crates = "core runtime cli"; phony_target("publish-dry", &[], &["cargo publish --dry-run -p core", "cargo publish --dry-run -p runtime", "cargo publish --dry-run -p cli"]); phony_target("publish", &["test", "lint", "publish-dry"], &["cargo publish -p core", "sleep 30", "cargo publish -p runtime", "sleep 30", "cargo publish -p cli"]); phony_target("version-check", &[], &["@cargo metadata --no-deps --format-version 1 | jq '.packages[].version'"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: publish-dry",
            ),
            CorpusEntry::new(
                "M-218",
                "makefile-e2e-testing",
                "End-to-end testing with test server lifecycle",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let port = "8080"; phony_target("e2e", &["e2e-start", "e2e-run", "e2e-stop"], &[]); phony_target("e2e-start", &["build"], &["./target/release/server &", "sleep 2"]); phony_target("e2e-run", &[], &["cargo test --test e2e -- --test-threads=1"]); phony_target("e2e-stop", &[], &["pkill -f 'target/release/server' || true"]); phony_target("build", &[], &["cargo build --release"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: e2e",
            ),
            CorpusEntry::new(
                "M-219",
                ...
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let image = "app:latest"; phony_target("scan", &["scan-image", "scan-fs", "scan-config"], &["@echo Security scan complete"]); phony_target("scan-image", &["build-image"], &["trivy image --severity HIGH,CRITICAL $(IMAGE)"]); phony_target("scan-fs", &[], &["trivy fs --severity HIGH,CRITICAL ."]); phony_target("scan-config", &[], &["trivy config ."]); phony_target("build-image", &[], &["docker build -t $(IMAGE) ."]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: scan",
            ),
            CorpusEntry::new(
                "M-220",
                "milestone-220-makefile",
                "Milestone 220: Full CI/CD pipeline — build, test, scan, deploy with rollback",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let env = "production"; phony_target("pipeline", &["lint", "test", "build", "scan", "deploy"], &["@echo Pipeline complete"]); phony_target("lint", &[], &["cargo clippy -- -D warnings", "cargo fmt -- --check"]); phony_target("test", &[], &["cargo test --workspace", "cargo test --doc"]); phony_target("build", &[], &["cargo build --release"]); phony_target("scan", &[], &["cargo audit", "cargo deny check"]); phony_target("deploy", &[], &["kubectl set image deployment/app app=$(IMAGE):$(TAG) -n $(ENV)"]); phony_target("rollback", &[], &["kubectl rollout undo deployment/app -n $(ENV)"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: pipeline",
            ),
        ];
        self.entries.extend(entries);
    ...
    // =========================================================================
    // Expansion Wave 23: Dockerfile D-201..D-220 (Adversarial/Production:
    //   security hardening, multi-stage optimization, exotic runtimes)
    // =========================================================================

    fn load_expansion13_dockerfile(&mut self) {
        let entries = vec![
            CorpusEntry::new(
                "D-201",
                "dockerfile-nonroot-hardened",
                "Security-hardened non-root container with read-only filesystem",
                CorpusFormat::Dockerfile,
                CorpusTier::Production,
                r#"fn main() { from_image_as("rust", "1.77-bookworm", "builder"); workdir("/app"); copy(".", "."); run(&["cargo build --release"]); from_image("gcr.io/distroless/cc-debian12", "nonroot"); copy_from("builder", "/app/target/release/app", "/app"); user("nonroot"); entrypoint(&["/app"]); } fn from_image_as(i: &str, t: &str, a: &str) {} fn from_image(i: &str, t: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn run(c: &[&str]) {} fn user(u: &str) {} fn entrypoint(e: &[&str]) {}"#,
                "USER nonroot",
            ),
            CorpusEntry::new(
                "D-202",
                ...
                CorpusFormat::Dockerfile,
                CorpusTier::Production,
                r#"fn main() { from_image_as("lukemathwalker/cargo-chef", "latest-rust-1.77", "chef"); workdir("/app"); from_image_as("chef", "", "planner"); copy(".", "."); run(&["cargo chef prepare --recipe-path recipe.json"]); from_image_as("chef", "", "builder"); copy_from("planner", "/app/recipe.json", "recipe.json"); run(&["cargo chef cook --release --recipe-path recipe.json"]); copy(".", "."); run(&["cargo build --release"]); from_image("debian", "bookworm-slim"); copy_from("builder", "/app/target/release/app", "/usr/local/bin/app"); entrypoint(&["app"]); } fn from_image_as(i: &str, t: &str, a: &str) {} fn from_image(i: &str, t: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn run(c: &[&str]) {} fn entrypoint(e: &[&str]) {}"#,
                "FROM lukemathwalker/cargo-chef",
            ),
            CorpusEntry::new(
                "D-203",
                "dockerfile-sidecar-pattern",
                "Sidecar pattern: app + envoy proxy in separate stages",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image_as("rust", "1.77", "app-builder"); workdir("/app"); copy(".", "."); run(&["cargo build --release"]); from_image_as("envoyproxy/envoy", "v1.29-latest", "proxy"); copy("envoy.yaml", "/etc/envoy/envoy.yaml"); from_image("debian", "bookworm-slim"); copy_from("app-builder", "/app/target/release/server", "/usr/local/bin/server"); copy_from("proxy", "/usr/local/bin/envoy", "/usr/local/bin/envoy"); expose(8080u16); expose(9901u16); cmd(&["server"]); } fn from_image_as(i: &str, t: &str, a: &str) {} fn from_image(i: &str, t: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn run(c: &[&str]) {} fn expose(p: u16) {} fn cmd(c: &[&str]) {}"#,
                "FROM envoyproxy/envoy",
            ),
            CorpusEntry::new(
                "D-204",
                "dockerfile-graalvm-native",
                "GraalVM native image for Java with multi-stage",
                CorpusFormat::Dockerfile,
                CorpusTier::Production,
                r#"fn main() { from_image_as("ghcr.io/graalvm/graalvm-community", "21", "builder"); workdir("/app"); copy(".", "."); run(&["./mvnw -Pnative native:compile -DskipTests"]); from_image("debian", "bookworm-slim"); copy_from("builder", "/app/target/native-image/app", "/app"); expose(8080u16); entrypoint(&["/app"]); } fn from_image_as(i: &str, t: &str, a: &str) {} fn from_image(i: &str, t: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn run(c: &[&str]) {} fn expose(p: u16) {} fn entrypoint(e: &[&str]) {}"#,
                "FROM ghcr.io/graalvm/graalvm-community",
            ),
            CorpusEntry::new(
                "D-205",
                "dockerfile-wolfi-apko",
                "Wolfi-based minimal container with apk",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image("cgr.dev/chainguard/wolfi-base", "latest"); run(&["apk add --no-cache ca-certificates"]); workdir("/app"); copy("app", "/app/app"); user("nonroot"); entrypoint(&["/app/app"]); } fn from_image(i: &str, t: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn user(u: &str) {} fn entrypoint(e: &[&str]) {}"#,
                "FROM cgr.dev/chainguard/wolfi-base",
            ),
            CorpusEntry::new(
                "D-206",
                "dockerfile-python-uv",
                "Python with uv package manager for fast installs",
                CorpusFormat::Dockerfile,
                CorpusTier::Production,
                r#"fn main() { from_image("python", "3.12-slim"); run(&["pip install uv"]); workdir("/app"); copy("pyproject.toml", "."); copy("uv.lock", "."); run(&["uv sync --frozen"]); copy("src/", "src/"); expose(8000u16); cmd(&["uv", "run", "uvicorn", "src.main:app", "--host", "0.0.0.0"]); } fn from_image(i: &str, t: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn expose(p: u16) {} fn cmd(c: &[&str]) {}"#,
                "FROM python:3.12-slim",
            ),
            CorpusEntry::new(
                "D-207",
                ...
                CorpusFormat::Dockerfile,
                CorpusTier::Standard,
                r#"fn main() { from_image("oven/bun", "1.1"); workdir("/app"); copy("package.json", "."); copy("bun.lockb", "."); run(&["bun install --frozen-lockfile"]); copy("src/", "src/"); expose(3000u16); cmd(&["bun", "run", "src/index.ts"]); } fn from_image(i: &str, t: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn run(c: &[&str]) {} fn expose(p: u16) {} fn cmd(c: &[&str]) {}"#,
                "FROM oven/bun:1.1",
            ),
            CorpusEntry::new(
                "D-208",
                "dockerfile-deno-compile",
                "Deno compile for single binary deployment",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image_as("denoland/deno", "1.40", "builder"); workdir("/app"); copy(".", "."); run(&["deno compile --allow-net --allow-read --output server main.ts"]); from_image("gcr.io/distroless/cc-debian12", ""); copy_from("builder", "/app/server", "/server"); expose(8000u16); entrypoint(&["/server"]); } fn from_image_as(i: &str, t: &str, a: &str) {} fn from_image(i: &str, t: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn run(c: &[&str]) {} fn expose(p: u16) {} fn entrypoint(e: &[&str]) {}"#,
                "FROM denoland/deno:1.40 AS builder",
            ),
            CorpusEntry::new(
                "D-209",
                "dockerfile-nginx-spa",
                "Nginx static SPA serving with custom config",
                CorpusFormat::Dockerfile,
                CorpusTier::Standard,
                r#"fn main() { from_image_as("node", "20-alpine", "builder"); workdir("/app"); copy("package.json", "."); copy("package-lock.json", "."); run(&["npm ci"]); copy(".", "."); run(&["npm run build"]); from_image("nginx", "alpine"); copy("nginx.conf", "/etc/nginx/conf.d/default.conf"); copy_from("builder", "/app/dist", "/usr/share/nginx/html"); expose(80u16); cmd(&["nginx", "-g", "daemon off;"]); } fn from_image_as(i: &str, t: &str, a: &str) {} fn from_image(i: &str, t: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn run(c: &[&str]) {} fn expose(p: u16) {} fn cmd(c: &[&str]) {}"#,
                "FROM node:20-alpine AS builder",
            ),
            CorpusEntry::new(
                "D-210",
                "dockerfile-trivy-scan-stage",
                "Build with integrated Trivy vulnerability scanning stage",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image_as("rust", "1.77", "builder"); workdir("/app"); copy(".", "."); run(&["cargo build --release"]); from_image_as("aquasec/trivy", "latest", "scanner"); copy_from("builder", "/app/target/release/app", "/app"); run(&["trivy rootfs --no-progress --severity HIGH,CRITICAL /"]); from_image("gcr.io/distroless/cc-debian12", ""); copy_from("builder", "/app/target/release/app", "/app"); entrypoint(&["/app"]); } fn from_image_as(i: &str, t: &str, a: &str) {} fn from_image(i: &str, t: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn run(c: &[&str]) {} fn entrypoint(e: &[&str]) {}"#,
                "FROM aquasec/trivy:latest AS scanner",
            ),
            CorpusEntry::new(
                "D-211",
                "dockerfile-wasm-wasmtime",
                "WebAssembly component compiled and run with wasmtime",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image_as("rust", "1.77", "builder"); run(&["rustup target add wasm32-wasi"]); workdir("/app"); copy(".", "."); run(&["cargo build --release --target wasm32-wasi"]); from_image("bytecodealliance/wasmtime", "latest"); copy_from("builder", "/app/target/wasm32-wasi/release/app.wasm", "/app.wasm"); entrypoint(&["wasmtime", "/app.wasm"]); } fn from_image_as(i: &str, t: &str, a: &str) {} fn from_image(i: &str, t: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn entrypoint(e: &[&str]) {}"#,
                "FROM bytecodealliance/wasmtime",
            ),
            CorpusEntry::new(
                "D-212",
                "dockerfile-earthly-style",
                "Earthly-inspired multi-target build",
                CorpusFormat::Dockerfile,
                CorpusTier::Production,
                r#"fn main() { from_image_as("rust", "1.77-bookworm", "base"); workdir("/app"); copy("Cargo.toml", "."); copy("Cargo.lock", "."); from_image_as("base", "", "deps"); run(&["cargo fetch"]); from_image_as("deps", "", "test"); copy("src/", "src/"); run(&["cargo test"]); from_image_as("deps", "", "release"); copy("src/", "src/"); run(&["cargo build --release"]); from_image("debian", "bookworm-slim"); copy_from("release", "/app/target/release/app", "/usr/local/bin/app"); entrypoint(&["app"]); } fn from_image_as(i: &str, t: &str, a: &str) {} fn from_image(i: &str, t: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn run(c: &[&str]) {} fn entrypoint(e: &[&str]) {}"#,
                "FROM rust:1.77-bookworm AS base",
            ),
            CorpusEntry::new(
                "D-213",
                "dockerfile-gpu-cuda",
                "CUDA GPU-enabled container for ML inference",
                CorpusFormat::Dockerfile,
                CorpusTier::Production,
                r#"fn main() { from_image("nvidia/cuda", "12.3.1-runtime-ubuntu22.04"); env("DEBIAN_FRONTEND", "noninteractive"); run(&["apt-get update", "apt-get install -y --no-install-recommends python3 python3-pip", "rm -rf /var/lib/apt/lists/*"]); workdir("/app"); copy("requirements.txt", "."); run(&["pip3 install --no-cache-dir -r requirements.txt"]); copy("model/", "model/"); copy("serve.py", "."); expose(8080u16); cmd(&["python3", "serve.py"]); } fn from_image(i: &str, t: &str) {} fn env(k: &str, v: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn expose(p: u16) {} fn cmd(c: &[&str]) {}"#,
                "FROM nvidia/cuda:12.3.1-runtime-ubuntu22.04",
            ),
            CorpusEntry::new(
                "D-214",
                "dockerfile-scratch-static",
                "Scratch-based container for fully static binary",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image_as("rust", "1.77-alpine", "builder"); run(&["apk add --no-cache musl-dev"]); workdir("/app"); copy(".", "."); run(&["cargo build --release --target x86_64-unknown-linux-musl"]); from_scratch(); copy_from("builder", "/app/target/x86_64-unknown-linux-musl/release/app", "/app"); entrypoint(&["/app"]); } fn from_image_as(i: &str, t: &str, a: &str) {} fn from_scratch() {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn entrypoint(e: &[&str]) {}"#,
                "FROM scratch",
            ),
            CorpusEntry::new(
                "D-215",
                "dockerfile-tauri-desktop",
                "Tauri desktop app build (Rust + web frontend)",
                CorpusFormat::Dockerfile,
                CorpusTier::Production,
                r#"fn main() { from_image("rust", "1.77-bookworm"); env("DEBIAN_FRONTEND", "noninteractive"); run(&["apt-get update", "apt-get install -y --no-install-recommends libwebkit2gtk-4.1-dev build-essential curl wget file libssl-dev libgtk-3-dev libayatana-appindicator3-dev librsvg2-dev", "rm -rf /var/lib/apt/lists/*"]); run(&["cargo install tauri-cli"]); workdir("/app"); copy(".", "."); run(&["cargo tauri build"]); cmd(&["ls", "-la", "target/release/bundle/"]); } fn from_image(i: &str, t: &str) {} fn env(k: &str, v: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn cmd(c: &[&str]) {}"#,
                "FROM rust:1.77-bookworm",
            ),
            CorpusEntry::new(
                "D-216",
                "dockerfile-multi-service-monolith",
                "Multi-service monolith: API + worker + scheduler in one image",
                CorpusFormat::Dockerfile,
                CorpusTier::Production,
                r#"fn main() { from_image_as("rust", "1.77", "builder"); workdir("/app"); copy(".", "."); run(&["cargo build --release --bin api", "cargo build --release --bin worker", "cargo build --release --bin scheduler"]); from_image("debian", "bookworm-slim"); run(&["apt-get update", "apt-get install -y --no-install-recommends ca-certificates", "rm -rf /var/lib/apt/lists/*"]); copy_from("builder", "/app/target/release/api", "/usr/local/bin/api"); copy_from("builder", "/app/target/release/worker", "/usr/local/bin/worker"); copy_from("builder", "/app/target/release/scheduler", "/usr/local/bin/scheduler"); expose(8080u16); cmd(&["api"]); } fn from_image_as(i: &str, t: &str, a: &str) {} fn from_image(i: &str, t: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn run(c: &[&str]) {} fn expose(p: u16) {} fn cmd(c: &[&str]) {}"#,
                "FROM rust:1.77 AS builder",
            ),
            CorpusEntry::new(
                "D-217",
                "dockerfile-label-oci",
                "OCI-compliant labels for container metadata",
                CorpusFormat::Dockerfile,
                CorpusTier::Standard,
                r#"fn main() { from_image("rust", "1.77-slim"); label("org.opencontainers.image.title", "myapp"); label("org.opencontainers.image.version", "1.0.0"); label("org.opencontainers.image.vendor", "myorg"); label("org.opencontainers.image.source", "https://github.com/org/app"); workdir("/app"); copy(".", "."); run(&["cargo build --release"]); cmd(&["./target/release/app"]); } fn from_image(i: &str, t: &str) {} fn label(k: &str, v: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn run(c: &[&str]) {} fn cmd(c: &[&str]) {}"#,
                "LABEL org.opencontainers.image.title=myapp",
            ),
            CorpusEntry::new(
                "D-218",
                "dockerfile-healthcheck-complex",
                "Complex healthcheck with curl and retry logic",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image("rust", "1.77-slim"); run(&["apt-get update", "apt-get install -y --no-install-recommends curl", "rm -rf /var/lib/apt/lists/*"]); workdir("/app"); copy(".", "."); run(&["cargo build --release"]); expose(8080u16); healthcheck("curl -f http://localhost:8080/health || exit 1", "30s", "10s"); cmd(&["./target/release/server"]); } fn from_image(i: &str, t: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn expose(p: u16) {} fn healthcheck(c: &str, i: &str, t: &str) {} fn cmd(c: &[&str]) {}"#,
                "HEALTHCHECK",
            ),
            CorpusEntry::new(
                "D-219",
                "dockerfile-kaniko-compat",
                "Kaniko-compatible build (no Docker daemon needed)",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image_as("rust", "1.77-bookworm", "builder"); env("CARGO_HOME", "/usr/local/cargo"); workdir("/app"); copy("Cargo.toml", "."); copy("Cargo.lock", "."); run(&["mkdir -p src", "echo 'fn main() {}' > src/main.rs", "cargo build --release", "rm -rf src"]); copy("src/", "src/"); run(&["cargo build --release"]); from_image("gcr.io/distroless/cc-debian12", ""); copy_from("builder", "/app/target/release/app", "/app"); entrypoint(&["/app"]); } fn from_image_as(i: &str, t: &str, a: &str) {} fn from_image(i: &str, t: &str) {} fn env(k: &str, v: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn run(c: &[&str]) {} fn entrypoint(e: &[&str]) {}"#,
                "ENV CARGO_HOME=/usr/local/cargo",
            ),
            CorpusEntry::new(
                "D-220",
                "milestone-220-docker",
                "Milestone 220: Full production Kubernetes-ready container — multi-stage + healthcheck + security + OCI labels",
                CorpusFormat::Dockerfile,
                CorpusTier::Production,
                r#"fn main() { from_image_as("rust", "1.77-bookworm", "builder"); workdir("/app"); copy("Cargo.toml", "."); copy("Cargo.lock", "."); run(&["mkdir -p src", "echo 'fn main() {}' > src/main.rs", "cargo build --release", "rm -rf src"]); copy("src/", "src/"); run(&["cargo build --release"]); from_image("debian", "bookworm-slim"); run(&["apt-get update", "apt-get install -y --no-install-recommends ca-certificates curl", "rm -rf /var/lib/apt/lists/*", "groupadd -r app", "useradd -r -g app -d /app app"]); label("org.opencontainers.image.title", "production-app"); label("org.opencontainers.image.version", "1.0.0"); copy_from("builder", "/app/target/release/app", "/usr/local/bin/app"); user("app"); expose(8080u16); healthcheck("curl -sf http://localhost:8080/healthz || exit 1", "15s", "5s"); entrypoint(&["app"]); } fn from_image_as(i: &str, t: &str, a: &str) {} fn from_image(i: &str, t: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn run(c: &[&str]) {} fn label(k: &str, v: &str) {} fn user(u: &str) {} fn expose(p: u16) {} fn healthcheck(c: &str, i: &str, t: &str) {} fn entrypoint(e: &[&str]) {}"#,
