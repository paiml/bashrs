...
impl CorpusRegistry {
    ...
    // =========================================================================
    // Expansion Wave 24: Bash B-531..B-570 (Adversarial: complex control flow,
    //   pathological quoting, one-liners, glob/wildcard patterns)
    // =========================================================================

    fn load_expansion19_bash(&mut self) {
        let entries = vec![
            // ── A. Control Flow: complex flow patterns ──────────────────────
            CorpusEntry::new(
                "B-531",
                "triple-nested-for-break",
                "Triple-nested for loops with conditional break in innermost",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let mut total = 0; for i in 1..6 { for j in 1..6 { for k in 1..6 { if i + j + k > 10 { break; } total += 1; } } } }"#,
                "for k in $(seq 1 5); do",
            ),
            CorpusEntry::new(
                "B-532",
                "while-multi-continue",
                "While loop with multiple continue conditions filtering items",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let mut i = 0; let mut sum = 0; while i < 50 { i += 1; if i % 3 == 0 { continue; } if i % 5 == 0 { continue; } if i % 7 == 0 { continue; } sum += i; } }"#,
                "while [ \"$i\" -lt 50 ]",
            ),
            CorpusEntry::new(
                "B-533",
                "elif-chain-arithmetic",
                "6-branch elif chain with arithmetic in conditions and bodies",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn categorize(x: i32) -> i32 { if x < -100 { -3 } else if x < -10 { -2 } else if x < 0 { -1 } else if x == 0 { 0 } else if x < 10 { 1 } else if x < 100 { 2 } else { 3 } } fn main() { let a = categorize(-200); let b = categorize(-5); let c = categorize(0); let d = categorize(50); let e = categorize(999); }"#,
                "categorize() {",
            ),
            CorpusEntry::new(
                "B-534",
                "for-case-accumulator",
                "For loop with case dispatch and per-branch accumulation",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let mut fizz = 0; let mut buzz = 0; let mut fizzbuzz = 0; let mut other = 0; for i in 1..31 { let fb = i % 15; let f = i % 3; let b = i % 5; if fb == 0 { fizzbuzz += 1; } else if f == 0 { fizz += 1; } else if b == 0 { buzz += 1; } else { other += 1; } } }"#,
                "for i in $(seq 1 30); do",
            ),
            CorpusEntry::new(
                "B-535",
                "nested-while-early-return",
                "Nested while loops with function early-return on inner condition",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn find_pair(target: i32) -> i32 { let mut i = 0; while i < 20 { let mut j = i; while j < 20 { if i + j == target { return i * 100 + j; } j += 1; } i += 1; } return -1; } fn main() { let p1 = find_pair(15); let p2 = find_pair(50); }"#,
                "find_pair() {",
            ),
            CorpusEntry::new(
                "B-536",
                "three-func-chain",
                "Three functions calling each other with conditional dispatch",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn transform(x: i32) -> i32 { if x > 0 { scale(x) } else { negate(x) } } fn scale(x: i32) -> i32 { x * 3 + offset(x) } fn negate(x: i32) -> i32 { 0 - x + offset(x) } fn offset(x: i32) -> i32 { if x % 2 == 0 { 1 } else { -1 } } fn main() { let a = transform(10); let b = transform(-5); let c = transform(0); }"#,
                "transform() {",
            ),
            CorpusEntry::new(
                "B-537",
                "pipeline-simulation",
                "Simulate pipeline data flow: generate, filter, reduce via variables",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn generate(n: i32) -> i32 { n * n } fn filter_pred(x: i32) -> bool { x % 2 == 0 } fn reduce(acc: i32, x: i32) -> i32 { acc + x } fn main() { let mut acc = 0; for i in 1..11 { let val = generate(i); if filter_pred(val) { acc = reduce(acc, val); } } }"#,
                "generate() {",
            ),
            CorpusEntry::new(
                "B-538",
                "case-range-classifier",
                "Case/match with range-like checks as sequential elif",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn http_status_class(code: i32) -> i32 { if code >= 100 && code < 200 { 1 } else if code >= 200 && code < 300 { 2 } else if code >= 300 && code < 400 { 3 } else if code >= 400 && code < 500 { 4 } else if code >= 500 && code < 600 { 5 } else { 0 } } fn main() { let a = http_status_class(200); let b = http_status_class(404); let c = http_status_class(503); let d = http_status_class(301); }"#,
                "http_status_class() {",
            ),
            CorpusEntry::new(
                "B-539",
                "multi-exit-loop",
                "Loop with three different exit conditions via break",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let mut i = 0; let mut sum = 0; let mut found_even_gt50 = false; while i < 200 { i += 7; sum += i; if sum > 500 { break; } if i > 100 { break; } if i % 2 == 0 && i > 50 { found_even_gt50 = true; break; } } }"#,
                "while [ \"$i\" -lt 200 ]",
            ),
            CorpusEntry::new(
                "B-540",
                "state-machine-5state",
                "5-state machine with transition table",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn transition(state: i32, input: i32) -> i32 { if state == 0 { if input > 0 { 1 } else { 4 } } else if state == 1 { if input > 5 { 2 } else { 0 } } else if state == 2 { if input > 10 { 3 } else { 1 } } else if state == 3 { if input == 0 { 0 } else { 4 } } else { if input > 0 { 1 } else { 0 } } } fn main() { let mut s = 0; s = transition(s, 3); s = transition(s, 8); s = transition(s, 15); s = transition(s, 0); s = transition(s, 1); }"#,
                "transition() {",
            ),
            CorpusEntry::new(
                "B-541",
                "interleaved-for-while",
                "For loop containing while loop for convergence at each step",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn main() { let mut result = 0; for i in 1..6 { let mut val = i * 10; while val > i { val = val - 3; } result += val; } }"#,
                "for i in $(seq 1 5); do",
            ),
            CorpusEntry::new(
                "B-542",
                "complex-boolean-while",
                "While loop with multi-variable compound boolean condition",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let mut a = 0; let mut b = 100; let mut c = 50; while a < b && c > 0 { a += 5; b = b - 3; c = c - 1; if a > c { b = b - 2; } } }"#,
                "while [ \"$a\" -lt \"$b\" ] && [ \"$c\" -gt 0 ]",
            ),
            CorpusEntry::new(
                "B-543",
                "simultaneous-variable-update",
                "Loop updating 4 variables simultaneously each iteration",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn main() { let mut x = 1; let mut y = 1; let mut z = 1; let mut w = 1; for _i in 0..10 { let nx = y + z; let ny = z + w; let nz = w + x; let nw = x + y; x = nx % 100; y = ny % 100; z = nz % 100; w = nw % 100; } }"#,
                "nx=$((y + z))",
            ),
            CorpusEntry::new(
                "B-544",
                "countdown-conditional-actions",
                "Countdown from 100 with conditional actions at thresholds",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn main() { let mut n = 100; let mut events = 0; while n > 0 { n = n - 1; if n == 75 { events += 1; } if n == 50 { events += 10; } if n == 25 { events += 100; } if n == 0 { events += 1000; } } }"#,
                "while [ \"$n\" -gt 0 ]",
            ),
            CorpusEntry::new(
                "B-545",
                "waterfall-processing",
                "Sequential waterfall pattern: 5 processing stages on data",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn stage1(x: i32) -> i32 { x + 10 } fn stage2(x: i32) -> i32 { x * 2 } fn stage3(x: i32) -> i32 { if x > 100 { x - 50 } else { x } } fn stage4(x: i32) -> i32 { x / 3 } fn stage5(x: i32) -> i32 { x % 17 } fn process(input: i32) -> i32 { let v1 = stage1(input); let v2 = stage2(v1); let v3 = stage3(v2); let v4 = stage4(v3); stage5(v4) } fn main() { let r1 = process(5); let r2 = process(42); let r3 = process(0); }"#,
                "stage1() {",
            ),
            // ── B. Pathological quoting ─────────────────────────────────────
            CorpusEntry::new(
                "B-546",
                "quote-embedded-single",
                "String with embedded apostrophe requiring careful quoting",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let msg = "don't panic"; }"#,
                "msg=",
            ),
            CorpusEntry::new(
                "B-547",
                "quote-embedded-double",
                "String with embedded double quotes",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let msg = "say \"hello\""; }"#,
                "msg=",
            ),
            CorpusEntry::new(
                "B-548",
                "quote-backslash-path",
                "String with backslashes simulating Windows path",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let path = "C:\\Users\\admin"; }"#,
                "path=",
            ),
            CorpusEntry::new(
                "B-549",
                "quote-dollar-literal",
                "String with literal dollar sign that must not expand",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let price = "$19.99"; }"#,
                "price=",
            ),
            CorpusEntry::new(
                "B-550",
                "quote-tilde-at-literal",
                "String with tilde and at-sign that are special in shell",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let user = "admin@host"; let home = "~user/dir"; }"#,
                "user=",
            ),
            CorpusEntry::new(
                "B-551",
                "quote-mixed-special",
                "String with multiple special shell characters mixed",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let chaos = "a&b|c;d>e<f"; }"#,
                "chaos=",
            ),
            CorpusEntry::new(
                "B-552",
                "quote-empty-strings",
                "Multiple empty string assignments",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let a = ""; let b = ""; let c = "non-empty"; }"#,
                "a=''",
            ),
            CorpusEntry::new(
                "B-553",
                "quote-parens-brackets",
                "String with parentheses and square brackets",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let expr = "(a+b)*[c-d]"; }"#,
                "expr=",
            ),
            CorpusEntry::new(
                "B-554",
                "quote-hash-comment-like",
                "String containing percent and exclamation marks",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let pct = "100%"; let bang = "hello!"; }"#,
                "pct=",
            ),
            CorpusEntry::new(
                "B-555",
                "quote-newline-tab-literal",
                "String with escaped newline and tab sequences",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let sep = "---"; let header = "col1\tcol2"; }"#,
                "sep='---'",
            ),
            ...
            CorpusEntry::new(
                "B-556",
                "oneliner-10-vars",
                "Dense one-liner: 10 variable assignments in quick succession",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let a = 1; let b = 2; let c = 3; let d = 4; let e = 5; let f = 6; let g = 7; let h = 8; let i = 9; let j = 10; }"#,
                "j='10'",
            ),
            CorpusEntry::new(
                "B-557",
                "oneliner-chain-arithmetic",
                "Dense chain of 8 arithmetic operations building on each other",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let a = 2; let b = a * 3; let c = b + a; let d = c * c; let e = d - b; let f = e / a; let g = f % 7; let h = g + d + c + b + a; }"#,
                "h=$(((g + d + c + b) + a))",
            ),
            CorpusEntry::new(
                "B-558",
                "oneliner-bool-chain",
                "Dense boolean chain: 6 boolean variables with compound logic",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let p = true; let q = false; let r = true; let s = p && !q; let t = s || r; let u = !t && q; }"#,
                "p=true",
            ),
            CorpusEntry::new(
                "B-559",
                "oneliner-nested-func-calls",
                "Dense nested function calls: f(g(h(x)))",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn add1(x: i32) -> i32 { x + 1 } fn double(x: i32) -> i32 { x * 2 } fn square(x: i32) -> i32 { x * x } fn main() { let a = add1(0); let b = double(a); let c = square(b); let d = add1(square(double(5))); }"#,
                "add1() {",
            ),
            CorpusEntry::new(
                "B-560",
                "oneliner-for-if-ternary",
                "Dense for loop with inline ternary-like if for each element",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn pick(x: i32) -> i32 { if x > 5 { x * 2 } else { x + 10 } } fn main() { let mut s = 0; for i in 0..10 { s += pick(i); } }"#,
                "pick() {",
            ),
            CorpusEntry::new(
                "B-561",
                "oneliner-multi-func-compact",
                "5 tiny functions defined and called in rapid succession",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn f1(x: i32) -> i32 { x + 1 } fn f2(x: i32) -> i32 { x - 1 } fn f3(x: i32) -> i32 { x * 2 } fn f4(x: i32) -> i32 { x / 2 } fn f5(x: i32) -> i32 { x % 3 } fn main() { let r = f5(f4(f3(f2(f1(100))))); }"#,
                "f1() {",
            ),
            CorpusEntry::new(
                "B-562",
                "oneliner-string-battery",
                "8 string variable assignments testing quoting variety",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let s1 = "hello"; let s2 = "world"; let s3 = "foo bar"; let s4 = "123"; let s5 = "true"; let s6 = ""; let s7 = "a-b_c.d"; let s8 = "/usr/local/bin"; }"#,
                "s8='/usr/local/bin'",
            ),
            CorpusEntry::new(
                "B-563",
                "oneliner-while-countdown-compact",
                "Compact countdown while loop with every-5th action",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn main() { let mut n = 50; let mut hits = 0; while n > 0 { if n % 5 == 0 { hits += 1; } n = n - 1; } }"#,
                "while [ \"$n\" -gt 0 ]",
            ),
            CorpusEntry::new(
                "B-564",
                "oneliner-gcd-lcm-compact",
                "GCD and LCM in compact iterative form",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn gcd(a: i32, b: i32) -> i32 { let mut x = a; let mut y = b; while y != 0 { let temp = y; y = x % y; x = temp; } x } fn lcm(a: i32, b: i32) -> i32 { a / gcd(a, b) * b } fn main() { let g = gcd(48, 18); let l = lcm(12, 8); }"#,
                "gcd() {",
            ),
            CorpusEntry::new(
                "B-565",
                "oneliner-fibonacci-nth",
                "Compact iterative Fibonacci returning nth term",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn fib(n: i32) -> i32 { let mut a = 0; let mut b = 1; let mut i = 0; while i < n { let tmp = a + b; a = b; b = tmp; i += 1; } a } fn main() { let f5 = fib(5); let f10 = fib(10); let f0 = fib(0); }"#,
                "fib() {",
            ),
            // ── D. Pathological globs and wildcards ─────────────────────────
            CorpusEntry::new(
                "B-566",
                "for-range-with-case",
                "For loop over range with case dispatch on modulo",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn classify_mod(x: u32) -> u32 { match x % 4 { 0 => { return 100; } 1 => { return 200; } 2 => { return 300; } _ => { return 400; } } } fn main() { let mut total = 0; for i in 0..12 { total += classify_mod(i); } }"#,
                "classify_mod() {",
            ),
            CorpusEntry::new(
                "B-567",
                "case-wildcard-star",
                "Match/case with wildcard-like default catch-all",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn classify_ext(ext: u32) -> u32 { match ext { 1 => { return 10; } 2 => { return 20; } 3 => { return 30; } 4 => { return 40; } _ => { return 0; } } } fn main() { let a = classify_ext(2); let b = classify_ext(99); }"#,
                "classify_ext() {",
            ),
            CorpusEntry::new(
                "B-568",
                "pattern-matching-cascade",
                "Cascade of pattern-like checks simulating glob matching",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn match_range(x: i32) -> i32 { if x >= 0 && x <= 9 { 1 } else if x >= 10 && x <= 99 { 2 } else if x >= 100 && x <= 999 { 3 } else if x >= 1000 && x <= 9999 { 4 } else { 5 } } fn main() { let a = match_range(5); let b = match_range(42); let c = match_range(500); let d = match_range(10000); }"#,
                "match_range() {",
            ),
            CorpusEntry::new(
                "B-569",
                "multi-case-dispatch",
                "Large case/match with 8 explicit arms and default",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn weekday(d: u32) -> u32 { match d { 0 => { return 1; } 1 => { return 2; } 2 => { return 3; } 3 => { return 4; } 4 => { return 5; } 5 => { return 10; } 6 => { return 10; } _ => { return 0; } } } fn main() { let m = weekday(0); let f = weekday(4); let s = weekday(5); let x = weekday(9); }"#,
                "weekday() {",
            ),
            CorpusEntry::new(
                "B-570",
                "milestone-570-comprehensive-flow",
                "Milestone 570: comprehensive flow — 4 functions, nested control, accumulation, pathological quoting",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn clamp(x: i32, lo: i32, hi: i32) -> i32 { if x < lo { lo } else if x > hi { hi } else { x } } fn abs_val(x: i32) -> i32 { if x < 0 { 0 - x } else { x } } fn sign(x: i32) -> i32 { if x > 0 { 1 } else if x < 0 { -1 } else { 0 } } fn main() { let mut total = 0; for i in 0..20 { let v = i * 7 - 50; let clamped = clamp(v, -20, 20); let av = abs_val(clamped); let s = sign(v); total += av * s; } let msg = "done"; }"#,
                "clamp() {",
            ),
        ];
        self.entries.extend(entries);
    ...
    // =========================================================================
    // Expansion Wave 25: Makefile M-221..M-240 (Adversarial: pathological
    //   quoting in recipes, shell escaping, complex variable expansion)
    // =========================================================================

    fn load_expansion12_makefile(&mut self) {
        let entries = vec![
            // ── B. Pathological quoting in Makefile recipes ─────────────────
            CorpusEntry::new(
                "M-221",
                "makefile-shell-single-quote",
                "Makefile recipe with single-quoted shell string",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("check-name", &[], &["test '$(NAME)' != ''"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: check-name",
            ),
            CorpusEntry::new(
                "M-222",
                "makefile-shell-double-quote",
                "Makefile recipe with double-quoted shell expansion",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { let version = "1.0.0"; phony_target("show-version", &[], &["echo \"Version: $(VERSION)\""]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: show-version",
            ),
            CorpusEntry::new(
                "M-223",
                "makefile-dollar-escape",
                "Makefile recipe with $$ for literal shell dollar sign",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("list-env", &[], &["for var in $$PATH $$HOME; do echo $$var; done"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: list-env",
            ),
            CorpusEntry::new(
                "M-224",
                "makefile-pipe-recipe",
                "Makefile recipe with pipe chain in shell command",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("count-lines", &[], &["find src -name '*.rs' | xargs wc -l | sort -n | tail -10"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: count-lines",
            ),
            CorpusEntry::new(
                "M-225",
                "makefile-redirect-recipe",
                "Makefile recipe with output redirect and stderr redirect",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("capture", &[], &["cargo test > test-output.txt 2>&1"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: capture",
            ),
            CorpusEntry::new(
                "M-226",
                "makefile-backslash-continuation",
                "Makefile with multi-step recipe simulating line continuation",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("deploy", &["build", "test"], &["echo 'Step 1: Package'", "echo 'Step 2: Upload'", "echo 'Step 3: Verify'"]); target("build", &[], &["cargo build --release"]); phony_target("test", &[], &["cargo test"]); } fn target(n: &str, d: &[&str], r: &[&str]) {} fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                "deploy: build test",
            ),
            CorpusEntry::new(
                "M-227",
                "makefile-semicolon-chain",
                "Makefile recipe with semicolon-chained shell commands",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("setup", &[], &["mkdir -p build; cd build; cmake .."]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: setup",
            ),
            CorpusEntry::new(
                "M-228",
                "makefile-conditional-recipe",
                "Makefile recipe with shell if-then-else inline",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("check-os", &[], &["if [ \"$$(uname)\" = 'Linux' ]; then echo linux; else echo other; fi"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: check-os",
            ),
            CorpusEntry::new(
                "M-229",
                "makefile-subshell-recipe",
                "Makefile recipe with subshell grouping",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("bench", &[], &["(cd bench && cargo bench) 2>&1 | tee bench.log"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: bench",
            ),
            CorpusEntry::new(
                "M-230",
                "makefile-wildcard-glob-recipe",
                "Makefile recipe using shell glob/wildcard patterns",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("clean-artifacts", &[], &["rm -f target/*.d target/*.o", "rm -rf target/debug/deps/*.rmeta"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: clean-artifacts",
            ),
            CorpusEntry::new(
                "M-231",
                "makefile-nested-quotes",
                "Makefile recipe with nested quoting levels",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("tag", &[], &["git tag -a v$(VERSION) -m 'Release version $(VERSION)'"]); let version = "2.0.0"; } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: tag",
            ),
            CorpusEntry::new(
                "M-232",
                "makefile-heredoc-like-recipe",
                "Makefile recipe writing multi-line content to file",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("gen-config", &[], &["echo '[database]' > config.toml", "echo 'host = \"localhost\"' >> config.toml", "echo 'port = 5432' >> config.toml"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: gen-config",
            ),
            CorpusEntry::new(
                "M-233",
                "makefile-ampersand-and",
                "Makefile recipe with && for conditional execution",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("safe-deploy", &[], &["cargo test && cargo build --release && ./deploy.sh"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: safe-deploy",
            ),
            CorpusEntry::new(
                "M-234",
                "makefile-or-fallback",
                "Makefile recipe with || fallback pattern",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("install", &[], &["which rg || cargo install ripgrep"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: install",
            ),
            CorpusEntry::new(
                "M-235",
                "makefile-devnull-suppress",
                "Makefile recipe with /dev/null suppression pattern",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("quiet-test", &[], &["cargo test 2>/dev/null || true"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: quiet-test",
            ),
            CorpusEntry::new(
                "M-236",
                "makefile-xargs-pipe",
                "Makefile recipe with find-xargs-pipe pattern",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { phony_target("fmt-check", &[], &["find . -name '*.rs' -not -path '*/target/*' | xargs rustfmt --check"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: fmt-check",
            ),
            CorpusEntry::new(
                "M-237",
                "makefile-multi-target-deps",
                "Makefile with complex dependency chain and quoting in recipes",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let src_dir = "src"; let build_dir = "target"; target("all", &["lib", "bin", "test"], &[]); target("lib", &[], &["cargo build --lib"]); target("bin", &["lib"], &["cargo build --bins"]); phony_target("test", &["lib"], &["cargo test --lib -- --nocapture"]); phony_target("clean", &[], &["rm -rf $(BUILD_DIR)"]); } fn target(n: &str, d: &[&str], r: &[&str]) {} fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                "all: lib bin test",
            ),
            CorpusEntry::new(
                "M-238",
                "makefile-pattern-rule-like",
                "Makefile with implicit-rule-style targets",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let cc = "gcc"; let cflags = "-Wall -O2"; target("%.o", &["%.c"], &["$(CC) $(CFLAGS) -c $< -o $@"]); target("app", &["main.o", "util.o"], &["$(CC) -o $@ $^"]); } fn target(n: &str, d: &[&str], r: &[&str]) {}"#,
                "CC := gcc",
            ),
            CorpusEntry::new(
                "M-239",
                "makefile-docker-compose-recipe",
                "Makefile orchestrating docker-compose with env vars and pipes",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let compose_file = "docker-compose.yml"; phony_target("up", &[], &["docker compose -f $(COMPOSE_FILE) up -d"]); phony_target("down", &[], &["docker compose -f $(COMPOSE_FILE) down --remove-orphans"]); phony_target("logs", &[], &["docker compose -f $(COMPOSE_FILE) logs -f 2>&1 | head -100"]); phony_target("ps", &[], &["docker compose -f $(COMPOSE_FILE) ps --format 'table {{.Name}}\t{{.Status}}'"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: up",
            ),
            CorpusEntry::new(
                "M-240",
                "milestone-240-makefile",
                "Milestone 240: Full CI/CD Makefile with pathological quoting, pipes, redirects, globs",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let cargo = "cargo"; let target = "target"; phony_target("all", &["lint", "test", "build"], &["echo 'All checks passed'"]); phony_target("lint", &[], &["$(CARGO) clippy --all-targets -- -D warnings 2>&1 | tee lint.log", "$(CARGO) fmt -- --check"]); phony_target("test", &[], &["$(CARGO) test --lib -- --nocapture 2>&1 | tee test.log || exit 1"]); target("build", &["lint", "test"], &["$(CARGO) build --release"]); phony_target("clean", &[], &["rm -rf $(TARGET)/debug $(TARGET)/release", "rm -f *.log"]); phony_target("coverage", &[], &["$(CARGO) llvm-cov --lib --lcov --output-path $(TARGET)/coverage/lcov.info"]); } fn target(n: &str, d: &[&str], r: &[&str]) {} fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: all",
            ),
        ];
        self.entries.extend(entries);
    ...
    // =========================================================================
    // Expansion Wave 26: Dockerfile D-221..D-230 (Adversarial: complex RUN
    //   chains with pipes, redirects, quoting in shell commands)
    // =========================================================================

    fn load_expansion14_dockerfile(&mut self) {
        let entries = vec![
            CorpusEntry::new(
                "D-221",
                "dockerfile-pipe-in-run",
                "Dockerfile RUN with pipe chain for package filtering",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image("debian", "bookworm-slim"); run(&["apt-get update", "apt-get install -y --no-install-recommends curl jq", "rm -rf /var/lib/apt/lists/*"]); run(&["curl -sL https://api.example.com/config | jq '.settings' > /etc/app/config.json"]); workdir("/app"); copy(".", "."); cmd(&["./app"]); } fn from_image(i: &str, t: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn cmd(c: &[&str]) {}"#,
                "FROM debian:bookworm-slim",
            ),
            CorpusEntry::new(
                "D-222",
                "dockerfile-redirect-devnull",
                "Dockerfile RUN suppressing output with /dev/null",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image("alpine", "3.19"); run(&["apk add --no-cache bash curl > /dev/null 2>&1"]); run(&["adduser -D -h /app appuser"]); user("appuser"); workdir("/app"); copy(".", "."); entrypoint(&["./start.sh"]); } fn from_image(i: &str, t: &str) {} fn run(c: &[&str]) {} fn user(u: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn entrypoint(e: &[&str]) {}"#,
                "FROM alpine:3.19",
            ),
            CorpusEntry::new(
                "D-223",
                "dockerfile-heredoc-config",
                "Dockerfile RUN with heredoc-style multi-line config generation",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image("nginx", "1.25-alpine"); run(&["echo 'server { listen 80; root /usr/share/nginx/html; }' > /etc/nginx/conf.d/default.conf"]); copy("dist/", "/usr/share/nginx/html/"); expose(80u16); cmd(&["nginx", "-g", "daemon off;"]); } fn from_image(i: &str, t: &str) {} fn run(c: &[&str]) {} fn copy(s: &str, d: &str) {} fn expose(p: u16) {} fn cmd(c: &[&str]) {}"#,
                "FROM nginx:1.25-alpine",
            ),
            CorpusEntry::new(
                "D-224",
                "dockerfile-env-quoting",
                "Dockerfile ENV with complex quoting and special characters",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image("python", "3.12-slim"); env("PYTHONDONTWRITEBYTECODE", "1"); env("PYTHONUNBUFFERED", "1"); env("PIP_NO_CACHE_DIR", "off"); env("APP_CONFIG", "/etc/app/config.json"); workdir("/app"); copy("requirements.txt", "."); run(&["pip install --no-cache-dir -r requirements.txt"]); copy(".", "."); cmd(&["python", "-m", "app"]); } fn from_image(i: &str, t: &str) {} fn env(k: &str, v: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn run(c: &[&str]) {} fn cmd(c: &[&str]) {}"#,
                "ENV PYTHONDONTWRITEBYTECODE=1",
            ),
            CorpusEntry::new(
                "D-225",
                "dockerfile-arg-conditional",
                "Dockerfile with ARG and conditional RUN based on build arg",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image("rust", "1.77-slim"); arg("PROFILE", "release"); workdir("/app"); copy("Cargo.toml", "."); copy("src/", "src/"); run(&["cargo build --$(PROFILE)"]); from_image("debian", "bookworm-slim"); copy_from("0", "/app/target/$(PROFILE)/app", "/usr/local/bin/app"); entrypoint(&["/usr/local/bin/app"]); } fn from_image(i: &str, t: &str) {} fn arg(k: &str, v: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn run(c: &[&str]) {} fn entrypoint(e: &[&str]) {}"#,
                "ARG PROFILE=release",
            ),
            CorpusEntry::new(
                "D-226",
                "dockerfile-glob-copy",
                "Dockerfile COPY with glob-like file selection patterns",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image("node", "20-alpine"); workdir("/app"); copy("package*.json", "./"); run(&["npm ci --production"]); copy("src/", "src/"); copy("public/", "public/"); copy("tsconfig.json", "."); run(&["npm run build"]); expose(3000u16); cmd(&["node", "dist/index.js"]); } fn from_image(i: &str, t: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn run(c: &[&str]) {} fn expose(p: u16) {} fn cmd(c: &[&str]) {}"#,
                "COPY package*.json ./",
            ),
            CorpusEntry::new(
                "D-227",
                "dockerfile-complex-healthcheck",
                "Dockerfile with complex healthcheck involving pipe and redirect",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image("golang", "1.22-alpine"); workdir("/app"); copy("go.mod", "."); copy("go.sum", "."); run(&["go mod download"]); copy(".", "."); run(&["go build -o /app/server"]); from_image("alpine", "3.19"); copy_from("0", "/app/server", "/usr/local/bin/server"); expose(8080u16); healthcheck("wget -qO- http://localhost:8080/health || exit 1", "10s", "3s"); cmd(&["/usr/local/bin/server"]); } fn from_image(i: &str, t: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn run(c: &[&str]) {} fn expose(p: u16) {} fn healthcheck(c: &str, i: &str, t: &str) {} fn cmd(c: &[&str]) {}"#,
                "HEALTHCHECK",
            ),
            CorpusEntry::new(
                "D-228",
                "dockerfile-multi-env-path",
                "Dockerfile with PATH manipulation and complex ENV",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image("ubuntu", "22.04"); env("DEBIAN_FRONTEND", "noninteractive"); env("LANG", "C.UTF-8"); run(&["apt-get update", "apt-get install -y --no-install-recommends build-essential python3 python3-pip", "rm -rf /var/lib/apt/lists/*"]); env("PATH", "/app/bin:$PATH"); workdir("/app"); copy(".", "."); run(&["pip3 install --no-cache-dir -e ."]); cmd(&["python3", "-m", "app"]); } fn from_image(i: &str, t: &str) {} fn env(k: &str, v: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn cmd(c: &[&str]) {}"#,
                "ENV DEBIAN_FRONTEND=noninteractive",
            ),
            CorpusEntry::new(
                "D-229",
                "dockerfile-shell-form-run",
                "Dockerfile RUN with shell form and complex quoting",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image("alpine", "3.19"); run(&["set -ex", "apk add --no-cache ca-certificates tzdata", "cp /usr/share/zoneinfo/UTC /etc/localtime", "echo 'UTC' > /etc/timezone"]); workdir("/app"); copy(".", "."); run(&["chmod +x /app/entrypoint.sh"]); entrypoint(&["/app/entrypoint.sh"]); } fn from_image(i: &str, t: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn entrypoint(e: &[&str]) {}"#,
                "FROM alpine:3.19",
            ),
            CorpusEntry::new(
                "D-230",
                "milestone-230-docker",
                "Milestone 230: Full production container with pipes, redirects, quoting, globs",
                CorpusFormat::Dockerfile,
                CorpusTier::Production,
                r#"fn main() { from_image_as("rust", "1.77-bookworm", "builder"); arg("FEATURES", "default"); workdir("/app"); copy("Cargo.toml", "."); copy("Cargo.lock", "."); run(&["mkdir -p src", "echo 'fn main() {}' > src/main.rs", "cargo build --release --features $(FEATURES) 2>&1 | tail -5", "rm -rf src"]); copy("src/", "src/"); run(&["cargo build --release --features $(FEATURES)"]); from_image("debian", "bookworm-slim"); env("RUST_LOG", "info"); run(&["apt-get update", "apt-get install -y --no-install-recommends ca-certificates curl", "rm -rf /var/lib/apt/lists/*", "groupadd -r app", "useradd -r -g app -d /app app"]); copy_from("builder", "/app/target/release/app", "/usr/local/bin/app"); user("app"); expose(8080u16); healthcheck("curl -sf http://localhost:8080/health > /dev/null 2>&1 || exit 1", "15s", "5s"); entrypoint(&["app"]); } fn from_image_as(i: &str, t: &str, a: &str) {} fn from_image(i: &str, t: &str) {} fn arg(k: &str, v: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn run(c: &[&str]) {} fn env(k: &str, v: &str) {} fn user(u: &str) {} fn expose(p: u16) {} fn healthcheck(c: &str, i: &str, t: &str) {} fn entrypoint(e: &[&str]) {}"#,
                "FROM rust:1.77-bookworm AS builder",
            ),
        ];
        self.entries.extend(entries);
    ...
    // =========================================================================
    // Expansion Wave 27: Bash B-571..B-620 — Round 2: EXTREME difficulty
    //   heredoc simulation, env vars, SSH, deep nesting, 8+ param functions
    // =========================================================================

    fn load_expansion20_bash(&mut self) {
        let entries = vec![
            // ── A. Extreme flow: 4+ nesting levels, 10+ functions ───────────
            CorpusEntry::new(
                "B-571",
                "quad-nested-for-accumulate",
                "4-level nested for loop with conditional accumulation",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let mut total = 0; for i in 1..4 { for j in 1..4 { for k in 1..4 { for l in 1..4 { if (i + j + k + l) % 2 == 0 { total += 1; } } } } } }"#,
                "for l in $(seq 1 3); do",
            ),
            CorpusEntry::new(
                "B-572",
                "ten-function-chain",
                "10 tiny functions chained together",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn f1(x: i32) -> i32 { x + 1 } fn f2(x: i32) -> i32 { x * 2 } fn f3(x: i32) -> i32 { x - 3 } fn f4(x: i32) -> i32 { x + 4 } fn f5(x: i32) -> i32 { x * 5 } fn f6(x: i32) -> i32 { x - 6 } fn f7(x: i32) -> i32 { x + 7 } fn f8(x: i32) -> i32 { x % 10 } fn f9(x: i32) -> i32 { x + 9 } fn f10(x: i32) -> i32 { x * 10 } fn main() { let r = f10(f9(f8(f7(f6(f5(f4(f3(f2(f1(0)))))))))); }"#,
                "f1() {",
            ),
            CorpusEntry::new(
                "B-573",
                "eight-param-function",
                "Function with 8 parameters doing complex computation",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn compute(a: i32, b: i32, c: i32, d: i32, e: i32, f: i32, g: i32, h: i32) -> i32 { (a + b) * (c - d) + (e * f) - (g + h) } fn main() { let r1 = compute(1, 2, 3, 4, 5, 6, 7, 8); let r2 = compute(10, 20, 30, 40, 50, 60, 70, 80); }"#,
                "compute() {",
            ),
            CorpusEntry::new(
                "B-574",
                "while-for-while-triple",
                "While containing for containing while — triple interleave",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let mut outer = 0; let mut total = 0; while outer < 3 { for i in 0..4 { let mut inner = 0; while inner < 2 { total += outer + i + inner; inner += 1; } } outer += 1; } }"#,
                "while [ \"$outer\" -lt 3 ]",
            ),
            CorpusEntry::new(
                "B-575",
                "cascading-returns",
                "Function with 6 return paths and complex conditions",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn classify(a: i32, b: i32, c: i32) -> i32 { if a > b && a > c { return 1; } if b > a && b > c { return 2; } if c > a && c > b { return 3; } if a == b && b == c { return 0; } if a == b { return 4; } return 5; } fn main() { let r1 = classify(10, 5, 3); let r2 = classify(1, 10, 3); let r3 = classify(1, 1, 10); let r4 = classify(5, 5, 5); let r5 = classify(7, 7, 3); let r6 = classify(3, 7, 7); }"#,
                "classify() {",
            ),
            CorpusEntry::new(
                "B-576",
                "deep-elif-10-branches",
                "10-branch elif chain with arithmetic in every branch",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn bucket(x: i32) -> i32 { if x < 10 { 0 } else if x < 20 { 1 } else if x < 30 { 2 } else if x < 40 { 3 } else if x < 50 { 4 } else if x < 60 { 5 } else if x < 70 { 6 } else if x < 80 { 7 } else if x < 90 { 8 } else { 9 } } fn main() { let mut sum = 0; for i in 0..10 { sum += bucket(i * 10 + 5); } }"#,
                "bucket() {",
            ),
            CorpusEntry::new(
                "B-577",
                "break-from-nested-match",
                "While loop with match inside that breaks on specific pattern",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let mut state = 0; let mut count = 0; while count < 100 { count += 1; let input = count % 5; match input { 0 => { state = 0; } 1 => { state = state + 1; } 2 => { state = state * 2; } 3 => { if state > 50 { break; } } _ => { state = state - 1; } } } }"#,
                "while [ \"$count\" -lt 100 ]",
            ),
            // ── E. Pathological environment variables ────────────────────────
            CorpusEntry::new(
                "B-578",
                "env-path-construction",
                "Build PATH-like variable from multiple segments",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn main() { let bin_dir = "/usr/local/bin"; let sbin_dir = "/usr/sbin"; let home_bin = "/home/user/bin"; let opt_bin = "/opt/tools/bin"; }"#,
                "bin_dir='/usr/local/bin'",
            ),
            CorpusEntry::new(
                "B-579",
                "env-config-battery",
                "12 environment-like config variables simulating .env file",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn main() { let app_name = "myservice"; let app_port = "8080"; let app_host = "0.0.0.0"; let db_host = "postgres.local"; let db_port = "5432"; let db_name = "production"; let db_user = "app_user"; let redis_url = "redis://cache:6379"; let log_level = "info"; let workers = "4"; let timeout = "30"; let secret = "abc123def456"; }"#,
                "app_name='myservice'",
            ),
            CorpusEntry::new(
                "B-580",
                ...
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn main() { let xdg_config_home = "/home/user/.config"; let xdg_data_home = "/home/user/.local/share"; let xdg_cache_home = "/home/user/.cache"; let xdg_state_home = "/home/user/.local/state"; let xdg_runtime_dir = "/run/user/1000"; }"#,
                "xdg_config_home='/home/user/.config'",
            ),
            CorpusEntry::new(
                "B-581",
                "env-java-opts",
                "Complex JVM-style options as string variable",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let java_opts = "-Xmx512m -Xms256m -XX:+UseG1GC"; let classpath = "/opt/app/lib/*:/opt/app/conf"; }"#,
                "java_opts='-Xmx512m -Xms256m -XX:+UseG1GC'",
            ),
            CorpusEntry::new(
                "B-582",
                "env-proxy-settings",
                "HTTP proxy configuration variables",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn main() { let http_proxy = "http://proxy.corp:8080"; let https_proxy = "http://proxy.corp:8080"; let no_proxy = "localhost,127.0.0.1,.corp.internal"; let ftp_proxy = "http://proxy.corp:8080"; }"#,
                "http_proxy='http://proxy.corp:8080'",
            ),
            // ── F. SSH-like operations and remote patterns ──────────────────
            CorpusEntry::new(
                "B-583",
                "ssh-config-vars",
                "SSH connection configuration variables",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn main() { let ssh_host = "bastion.prod.example.com"; let ssh_port = "22"; let ssh_user = "deploy"; let ssh_key = "/home/deploy/.ssh/id_ed25519"; let ssh_opts = "-o StrictHostKeyChecking=no -o ConnectTimeout=10"; let remote_dir = "/opt/app/current"; }"#,
                "ssh_host='bastion.prod.example.com'",
            ),
            CorpusEntry::new(
                "B-584",
                "ssh-tunnel-params",
                "SSH tunnel forwarding parameters",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let local_port = "5433"; let remote_host = "db.internal"; let remote_port = "5432"; let jump_host = "bastion.prod"; let bind_addr = "127.0.0.1"; }"#,
                "local_port='5433'",
            ),
            CorpusEntry::new(
                "B-585",
                "scp-rsync-paths",
                "SCP and rsync-style source/destination path variables",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn main() { let src_local = "/var/log/app/"; let dst_remote = "deploy@prod:/backup/logs/"; let rsync_flags = "-avz --delete --exclude=*.tmp"; let ssh_cmd = "ssh -i /home/deploy/.ssh/id_ed25519 -p 2222"; }"#,
                "src_local='/var/log/app/'",
            ),
            // ── D2. Heredoc simulation (multi-line config generation) ───────
            CorpusEntry::new(
                "B-586",
                "heredoc-config-vars",
                "Simulate heredoc: generate config file content via variables",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let line1 = "[database]"; let line2 = "host = localhost"; let line3 = "port = 5432"; let line4 = "name = mydb"; let line5 = ""; let line6 = "[server]"; let line7 = "port = 8080"; let line8 = "workers = 4"; }"#,
                "line1='[database]'",
            ),
            CorpusEntry::new(
                "B-587",
                "heredoc-yaml-config",
                "YAML-like config generation via string variables",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let y1 = "apiVersion: v1"; let y2 = "kind: ConfigMap"; let y3 = "metadata:"; let y4 = "  name: app-config"; let y5 = "data:"; let y6 = "  DB_HOST: postgres"; let y7 = "  DB_PORT: '5432'"; }"#,
                "y1='apiVersion: v1'",
            ),
            CorpusEntry::new(
                "B-588",
                "heredoc-nginx-config",
                "Nginx config generation via string variables",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let l1 = "server {"; let l2 = "    listen 80;"; let l3 = "    server_name example.com;"; let l4 = "    location / {"; let l5 = "        proxy_pass http://backend:8080;"; let l6 = "    }"; let l7 = "}"; }"#,
                "l1='server {'",
            ),
            CorpusEntry::new(
                "B-589",
                "heredoc-systemd-unit",
                "Systemd unit file content via variables",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let u1 = "[Unit]"; let u2 = "Description=My Application"; let u3 = "After=network.target"; let u4 = ""; let u5 = "[Service]"; let u6 = "Type=simple"; let u7 = "ExecStart=/usr/local/bin/app"; let u8 = "Restart=always"; let u9 = ""; let u10 = "[Install]"; let u11 = "WantedBy=multi-user.target"; }"#,
                "u1='[Unit]'",
            ),
            // ── C2. Extreme one-liners ──────────────────────────────────────
            CorpusEntry::new(
                "B-590",
                "oneliner-20var-chain",
                "20 variables each depending on the previous",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let a = 1; let b = a + 1; let c = b + 1; let d = c + 1; let e = d + 1; let f = e + 1; let g = f + 1; let h = g + 1; let i = h + 1; let j = i + 1; let k = j + 1; let l = k + 1; let m = l + 1; let n = m + 1; let o = n + 1; let p = o + 1; let q = p + 1; let r = q + 1; let s = r + 1; let t = s + 1; }"#,
                "t=$((s + 1))",
            ),
            CorpusEntry::new(
                "B-591",
                "oneliner-complex-arithmetic",
                "Single expression with 10 arithmetic operations",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let x = 2; let y = 3; let z = (x + y) * (x - y) + x * y * (x + 1) - (y * y) + x; }"#,
                "z=",
            ),
            CorpusEntry::new(
                "B-592",
                "oneliner-15-string-vars",
                "15 string variables simulating command construction",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let cmd = "rsync"; let f1 = "-a"; let f2 = "-v"; let f3 = "-z"; let f4 = "--delete"; let f5 = "--exclude=.git"; let f6 = "--exclude=node_modules"; let f7 = "--exclude=target"; let f8 = "-e"; let f9 = "ssh -p 2222"; let src = "/opt/app/"; let dst = "prod:/opt/app/"; let log = "/var/log/rsync.log"; let dry = "--dry-run"; let timeout = "--timeout=60"; }"#,
                "cmd='rsync'",
            ),
            CorpusEntry::new(
                "B-593",
                "oneliner-modular-arithmetic",
                "Modular arithmetic chain with remainder operations",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let a = 1000003; let b = a % 97; let c = b * b % 97; let d = c * c % 97; let e = d * b % 97; let f = e + d + c + b; }"#,
                "b=$((a % 97))",
            ),
            // ── B2. Pathological quoting round 2 ────────────────────────────
            CorpusEntry::new(
                "B-594",
                "quote-url-special",
                ...
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let url = "https://api.example.com/v1/search?q=hello+world&limit=10"; }"#,
                "url=",
            ),
            CorpusEntry::new(
                "B-595",
                "quote-regex-like",
                "Regex-like pattern string with special chars",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let pattern = "^[a-zA-Z0-9_]+$"; }"#,
                "pattern=",
            ),
            CorpusEntry::new(
                "B-596",
                "quote-json-like",
                "JSON-like string content needing careful escaping",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let json = "{\"name\":\"test\",\"value\":42}"; }"#,
                "json=",
            ),
            CorpusEntry::new(
                "B-597",
                "quote-sql-like",
                "SQL-like string with single quotes and keywords",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let query = "SELECT * FROM users WHERE active = 1"; }"#,
                "query='SELECT * FROM users WHERE active = 1'",
            ),
            CorpusEntry::new(
                "B-598",
                "quote-multiword-args",
                "Multiple space-containing strings needing quoting",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let name = "John Doe"; let addr = "123 Main Street"; let city = "New York"; let zip = "10001"; }"#,
                "name='John Doe'",
            ),
            CorpusEntry::new(
                "B-599",
                "quote-cron-expression",
                "Cron expression string with asterisks and slashes",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let cron = "*/5 * * * *"; let cron2 = "0 2 * * 1-5"; }"#,
                "cron=",
            ),
            CorpusEntry::new(
                "B-600",
                "milestone-600-extreme",
                "Milestone 600: extreme — 6 functions, deep nesting, pathological strings, env vars",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn pow_mod(base: i32, exp: i32, modulus: i32) -> i32 { let mut result = 1; let mut b = base % modulus; let mut e = exp; while e > 0 { if e % 2 == 1 { result = result * b % modulus; } e = e / 2; b = b * b % modulus; } result } fn gcd(a: i32, b: i32) -> i32 { let mut x = a; let mut y = b; while y != 0 { let t = y; y = x % y; x = t; } x } fn lcm(a: i32, b: i32) -> i32 { a / gcd(a, b) * b } fn abs_val(x: i32) -> i32 { if x < 0 { 0 - x } else { x } } fn sign(x: i32) -> i32 { if x > 0 { 1 } else if x < 0 { -1 } else { 0 } } fn clamp(x: i32, lo: i32, hi: i32) -> i32 { if x < lo { lo } else if x > hi { hi } else { x } } fn main() { let pm = pow_mod(2, 10, 1000); let g = gcd(48, 18); let l = lcm(12, 8); let a = abs_val(-42); let s = sign(-7); let c = clamp(150, 0, 100); let cfg = "/etc/app/config"; }"#,
                "pow_mod() {",
            ),
            // ── Extreme: deeper nesting, more params ────────────────────────
            CorpusEntry::new(
                "B-601",
                "nested-match-in-for-in-while",
                "While -> for -> match triple nesting",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let mut gen = 0; let mut score = 0; while gen < 5 { for i in 0..10 { let category = i % 4; match category { 0 => { score += 10; } 1 => { score += 5; } 2 => { score += 1; } _ => { score = score; } } } gen += 1; } }"#,
                "while [ \"$gen\" -lt 5 ]",
            ),
            CorpusEntry::new(
                "B-602",
                "func-mutual-dispatch",
                "Two functions dispatching to each other via condition",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn process_a(x: i32) -> i32 { if x > 100 { return x; } x * 2 + 1 } fn process_b(x: i32) -> i32 { if x > 100 { return x; } x * 3 - 1 } fn dispatch(x: i32, mode: i32) -> i32 { if mode == 0 { process_a(x) } else { process_b(x) } } fn main() { let r1 = dispatch(10, 0); let r2 = dispatch(10, 1); let r3 = dispatch(50, 0); let r4 = dispatch(50, 1); }"#,
                "dispatch() {",
            ),
            CorpusEntry::new(
                "B-603",
                "loop-with-6-accumulators",
                "Single loop updating 6 accumulators based on conditions",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let mut c1 = 0; let mut c2 = 0; let mut c3 = 0; let mut c4 = 0; let mut c5 = 0; let mut c6 = 0; for i in 1..101 { if i % 2 == 0 { c1 += 1; } if i % 3 == 0 { c2 += 1; } if i % 5 == 0 { c3 += 1; } if i % 7 == 0 { c4 += 1; } if i % 11 == 0 { c5 += 1; } if i % 13 == 0 { c6 += 1; } } }"#,
                "for i in $(seq 1 100); do",
            ),
            CorpusEntry::new(
                "B-604",
                "double-while-convergence",
                "Two converging while loops running in sequence",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn main() { let mut lo = 0; let mut hi = 1000; while hi - lo > 1 { let mid = (lo + hi) / 2; if mid * mid > 500 { hi = mid; } else { lo = mid; } } let sqrt_approx = lo; let mut val = 1000; while val > 1 { val = val * 7 / 10; } }"#,
                "while [ $((hi - lo)) -gt 1 ]",
            ),
            CorpusEntry::new(
                "B-605",
                "seven-param-function",
                "Function with 7 parameters and complex body",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn weighted_sum(a: i32, b: i32, c: i32, wa: i32, wb: i32, wc: i32, divisor: i32) -> i32 { (a * wa + b * wb + c * wc) / divisor } fn main() { let r1 = weighted_sum(10, 20, 30, 3, 2, 1, 6); let r2 = weighted_sum(100, 200, 300, 1, 1, 1, 3); }"#,
                "weighted_sum() {",
            ),
            // ── More pathological strings ───────────────────────────────────
            CorpusEntry::new(
                "B-606",
                "quote-docker-run-cmd",
                "Docker run command string with flags and quotes",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let docker_cmd = "docker run -d --name=app -p 8080:80 -v /data:/app/data --restart=unless-stopped myimage:latest"; }"#,
                "docker_cmd=",
            ),
            CorpusEntry::new(
                "B-607",
                "quote-awk-program",
                "AWK program string needing careful quoting",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let awk_prog = "{print $1, $3}"; }"#,
                "awk_prog=",
            ),
            CorpusEntry::new(
                "B-608",
                "quote-sed-expression",
                "Sed substitution expression string",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let sed_expr = "s/old/new/g"; let sed2 = "s|/usr/local|/opt|g"; }"#,
                "sed_expr=",
            ),
            CorpusEntry::new(
                "B-609",
                "quote-find-delete",
                "Find command string with -delete flag",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let find_cmd = "find . -name '*.log' -mtime +7 -delete"; }"#,
                "find_cmd=",
            ),
            CorpusEntry::new(
                "B-610",
                "quote-connection-string",
                "Database connection string with special chars",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn main() { let conn = "postgresql://user:p@ssw0rd@db.host:5432/mydb?sslmode=require"; }"#,
                "conn=",
            ),
            // ── Extreme arithmetic and logic ────────────────────────────────
            CorpusEntry::new(
                "B-611",
                ...
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn mod_exp(base: i32, exp: i32, m: i32) -> i32 { let mut result = 1; let mut b = base % m; let mut e = exp; while e > 0 { if e % 2 == 1 { result = result * b % m; } e = e / 2; b = b * b % m; } result } fn main() { let r1 = mod_exp(2, 10, 1000); let r2 = mod_exp(3, 20, 100); let r3 = mod_exp(7, 7, 13); }"#,
                "mod_exp() {",
            ),
            CorpusEntry::new(
                "B-612",
                "extended-gcd",
                "Extended GCD computing Bezout coefficients iteratively",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn ext_gcd_r(a: i32, b: i32) -> i32 { let mut old_r = a; let mut r = b; let mut old_s = 1; let mut s = 0; while r != 0 { let q = old_r / r; let temp_r = r; r = old_r - q * r; old_r = temp_r; let temp_s = s; s = old_s - q * s; old_s = temp_s; } old_r } fn main() { let g = ext_gcd_r(240, 46); }"#,
                "ext_gcd_r() {",
            ),
            CorpusEntry::new(
                "B-613",
                "sieve-of-eratosthenes-sim",
                "Sieve simulation counting composites via nested loops",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn is_composite(n: i32) -> bool { if n < 2 { return true; } let mut d = 2; while d * d <= n { if n % d == 0 { return true; } d += 1; } false } fn count_composites(limit: i32) -> i32 { let mut count = 0; let mut n = 4; while n <= limit { if is_composite(n) { count += 1; } n += 1; } count } fn main() { let c = count_composites(50); }"#,
                "is_composite() {",
            ),
            CorpusEntry::new(
                "B-614",
                ...
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn isqrt(n: i32) -> i32 { if n < 2 { return n; } let mut x = n; let mut y = (x + 1) / 2; while y < x { x = y; y = (x + n / x) / 2; } x } fn main() { let s1 = isqrt(0); let s2 = isqrt(1); let s3 = isqrt(4); let s4 = isqrt(10); let s5 = isqrt(100); let s6 = isqrt(1000); }"#,
                "isqrt() {",
            ),
            CorpusEntry::new(
                "B-615",
                "tower-of-hanoi-iterative",
                "Tower of Hanoi iterative move counting",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn hanoi_moves(n: i32) -> i32 { let mut moves = 1; let mut i = 0; while i < n { moves = moves * 2; i += 1; } moves - 1 } fn main() { let m1 = hanoi_moves(1); let m3 = hanoi_moves(3); let m5 = hanoi_moves(5); let m10 = hanoi_moves(10); }"#,
                "hanoi_moves() {",
            ),
            // ── D2. Glob/wildcard simulation round 2 ────────────────────────
            CorpusEntry::new(
                "B-616",
                "case-12-arms",
                "Case/match with 12 explicit arms for month lookup",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn month_days(m: u32) -> u32 { match m { 1 => { return 31; } 2 => { return 28; } 3 => { return 31; } 4 => { return 30; } 5 => { return 31; } 6 => { return 30; } 7 => { return 31; } 8 => { return 31; } 9 => { return 30; } 10 => { return 31; } 11 => { return 30; } _ => { return 31; } } } fn main() { let mut total = 0; for m in 1..13 { total += month_days(m); } }"#,
                "month_days() {",
            ),
            CorpusEntry::new(
                "B-617",
                "multi-range-classifier",
                "Classifier with overlapping range checks via compound conditions",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn classify_temp(t: i32) -> i32 { if t <= -40 { 0 } else if t <= -20 { 1 } else if t <= 0 { 2 } else if t <= 10 { 3 } else if t <= 20 { 4 } else if t <= 30 { 5 } else if t <= 40 { 6 } else { 7 } } fn main() { let a = classify_temp(-50); let b = classify_temp(-10); let c = classify_temp(5); let d = classify_temp(25); let e = classify_temp(45); }"#,
                "classify_temp() {",
            ),
            CorpusEntry::new(
                "B-618",
                "priority-dispatch",
                "Priority-based dispatch with 5 priority levels",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn priority(p: i32) -> i32 { if p == 0 { 1000 } else if p == 1 { 500 } else if p == 2 { 100 } else if p == 3 { 10 } else if p == 4 { 1 } else { 0 } } fn schedule(n: i32) -> i32 { let mut total = 0; for i in 0..n { let p = i % 5; total += priority(p); } total } fn main() { let s = schedule(20); }"#,
                "priority() {",
            ),
            CorpusEntry::new(
                "B-619",
                "hash-simulation",
                "Simple hash function via arithmetic operations",
                CorpusFormat::Bash,
                CorpusTier::Adversarial,
                r#"fn simple_hash(key: i32, table_size: i32) -> i32 { let mut h = key; h = ((h * 2654435761) / 65536) % table_size; if h < 0 { h = 0 - h; } h } fn main() { let h1 = simple_hash(42, 16); let h2 = simple_hash(100, 16); let h3 = simple_hash(0, 16); let h4 = simple_hash(999, 32); }"#,
                "simple_hash() {",
            ),
            CorpusEntry::new(
                "B-620",
                "milestone-620-extreme-stress",
                "Milestone 620: extreme stress — 8 functions, deep nesting, env/ssh/heredoc patterns",
                CorpusFormat::Bash,
                CorpusTier::Production,
                r#"fn abs_val(x: i32) -> i32 { if x < 0 { 0 - x } else { x } } fn min2(a: i32, b: i32) -> i32 { if a < b { a } else { b } } fn max2(a: i32, b: i32) -> i32 { if a > b { a } else { b } } fn clamp(x: i32, lo: i32, hi: i32) -> i32 { min2(max2(x, lo), hi) } fn gcd(a: i32, b: i32) -> i32 { let mut x = a; let mut y = b; while y != 0 { let t = y; y = x % y; x = t; } x } fn is_prime(n: i32) -> bool { if n < 2 { return false; } let mut d = 2; while d * d <= n { if n % d == 0 { return false; } d += 1; } true } fn count_primes_in_range(lo: i32, hi: i32) -> i32 { let mut count = 0; let mut n = lo; while n <= hi { if is_prime(n) { count += 1; } n += 1; } count } fn main() { let a = abs_val(-99); let clamped = clamp(200, 0, 100); let g = gcd(360, 48); let primes = count_primes_in_range(2, 50); let host = "prod.example.com"; let port = "8443"; let log_dir = "/var/log/app"; }"#,
                "abs_val() {",
            ),
        ];
        self.entries.extend(entries);
    ...
    // =========================================================================
    // Expansion Wave 28: Makefile M-241..M-260 — Round 2: EXTREME quoting,
    //   heredoc, SSH operations, complex variable expansion in recipes
    // =========================================================================

    fn load_expansion13_makefile(&mut self) {
        let entries = vec![
            // ── Heredoc-like Makefile patterns ──────────────────────────────
            CorpusEntry::new(
                "M-241",
                "makefile-heredoc-systemd",
                "Makefile recipe generating systemd unit file content",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("gen-service", &[], &["echo '[Unit]' > /tmp/app.service", "echo 'Description=App' >> /tmp/app.service", "echo '[Service]' >> /tmp/app.service", "echo 'ExecStart=/usr/bin/app' >> /tmp/app.service", "echo '[Install]' >> /tmp/app.service", "echo 'WantedBy=multi-user.target' >> /tmp/app.service"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: gen-service",
            ),
            CorpusEntry::new(
                "M-242",
                "makefile-heredoc-nginx",
                "Makefile recipe generating nginx config with redirects",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("gen-nginx", &[], &["echo 'server {' > /etc/nginx/app.conf", "echo '  listen 80;' >> /etc/nginx/app.conf", "echo '  return 301 https://$$host$$request_uri;' >> /etc/nginx/app.conf", "echo '}' >> /etc/nginx/app.conf"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: gen-nginx",
            ),
            // ── SSH in Makefile ────────────────────────────────────────────
            CorpusEntry::new(
                "M-243",
                "makefile-ssh-deploy",
                "Makefile with SSH deploy recipe and complex quoting",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { let ssh_host = "deploy@prod.example.com"; let ssh_key = "~/.ssh/deploy_key"; let remote_dir = "/opt/app"; phony_target("deploy", &["build"], &["scp -i $(SSH_KEY) target/release/app $(SSH_HOST):$(REMOTE_DIR)/", "ssh -i $(SSH_KEY) $(SSH_HOST) 'systemctl restart app'"]); target("build", &[], &["cargo build --release"]); } fn target(n: &str, d: &[&str], r: &[&str]) {} fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                "deploy: build",
            ),
            CorpusEntry::new(
                "M-244",
                "makefile-ssh-tunnel",
                "Makefile with SSH tunnel and port forwarding recipe",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { let db_host = "db.internal"; let jump_host = "bastion.prod"; phony_target("db-tunnel", &[], &["ssh -fN -L 5433:$(DB_HOST):5432 $(JUMP_HOST)"]); phony_target("kill-tunnel", &[], &["pkill -f 'ssh.*5433.*$(DB_HOST)' || true"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: db-tunnel",
            ),
            CorpusEntry::new(
                "M-245",
                "makefile-rsync-deploy",
                "Makefile with rsync deploy using complex exclude patterns",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let remote = "deploy@prod:/opt/app/"; let ssh_cmd = "ssh -i ~/.ssh/deploy_key -p 2222"; phony_target("sync", &[], &["rsync -avz --delete --exclude='.git' --exclude='target' --exclude='*.log' -e '$(SSH_CMD)' ./ $(REMOTE)"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: sync",
            ),
            // ── Extreme quoting in Makefile ─────────────────────────────────
            CorpusEntry::new(
                "M-246",
                "makefile-json-output",
                "Makefile recipe generating JSON output with quoting",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { let version = "1.0.0"; phony_target("version-json", &[], &["echo '{\"version\": \"$(VERSION)\", \"build\": \"release\"}' > version.json"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: version-json",
            ),
            CorpusEntry::new(
                "M-247",
                "makefile-awk-recipe",
                "Makefile recipe with embedded AWK program",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("coverage-report", &[], &["cargo llvm-cov --lib 2>&1 | awk '/TOTAL/{print $$NF}'"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: coverage-report",
            ),
            CorpusEntry::new(
                "M-248",
                "makefile-sed-recipe",
                "Makefile recipe with sed substitution",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { let version = "2.0.0"; phony_target("bump-version", &[], &["sed -i 's/^version = .*/version = \"$(VERSION)\"/' Cargo.toml"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: bump-version",
            ),
            CorpusEntry::new(
                "M-249",
                "makefile-shell-for-loop",
                "Makefile recipe with shell for loop and variable expansion",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("test-all", &[], &["for dir in api web cli; do echo \"Testing $$dir\"; cargo test -p $$dir; done"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: test-all",
            ),
            CorpusEntry::new(
                "M-250",
                "milestone-250-makefile",
                "Milestone 250: extreme Makefile with SSH, heredoc, pipes, AWK, sed",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let version = "3.0.0"; let remote = "prod.example.com"; phony_target("all", &["lint", "test", "build", "package"], &["echo 'Pipeline complete'"]); phony_target("lint", &[], &["cargo clippy -- -D warnings 2>&1 | tee lint.log"]); phony_target("test", &[], &["cargo test 2>&1 | tee test.log"]); target("build", &["lint", "test"], &["cargo build --release"]); phony_target("package", &["build"], &["tar czf app-$(VERSION).tar.gz -C target/release app"]); phony_target("deploy", &["package"], &["scp app-$(VERSION).tar.gz $(REMOTE):/opt/releases/", "ssh $(REMOTE) 'cd /opt/releases && tar xzf app-$(VERSION).tar.gz && systemctl restart app'"]); phony_target("clean", &[], &["rm -rf target *.log *.tar.gz"]); } fn target(n: &str, d: &[&str], r: &[&str]) {} fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                "all: lint test build package",
            ),
            // ── More extreme Makefile patterns ──────────────────────────────
            CorpusEntry::new(
                "M-251",
                "makefile-env-file-gen",
                "Makefile recipe generating .env file with redirects",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { let db_host = "localhost"; let db_port = "5432"; phony_target("gen-env", &[], &["echo 'DB_HOST=$(DB_HOST)' > .env", "echo 'DB_PORT=$(DB_PORT)' >> .env", "echo 'REDIS_URL=redis://localhost:6379' >> .env", "echo 'LOG_LEVEL=info' >> .env"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: gen-env",
            ),
            CorpusEntry::new(
                "M-252",
                "makefile-docker-build-args",
                "Makefile with docker build using --build-arg and complex quoting",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { let version = "1.0.0"; let registry = "ghcr.io/org"; phony_target("docker-build", &[], &["docker build --build-arg VERSION=$(VERSION) --build-arg BUILD_DATE=$$(date -u +%Y-%m-%dT%H:%M:%SZ) -t $(REGISTRY)/app:$(VERSION) -t $(REGISTRY)/app:latest ."]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: docker-build",
            ),
            CorpusEntry::new(
                "M-253",
                "makefile-curl-post",
                "Makefile recipe with curl POST and JSON body",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { let api_url = "https://api.example.com"; phony_target("notify-deploy", &[], &["curl -sS -X POST $(API_URL)/webhooks/deploy -H 'Content-Type: application/json' -d '{\"status\": \"deployed\"}' -o /dev/null"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: notify-deploy",
            ),
            CorpusEntry::new(
                "M-254",
                "makefile-parallel-test",
                "Makefile with parallel test execution and log collection",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { phony_target("test-parallel", &[], &["cargo test --lib -- --test-threads=4 2>&1 | tee test-lib.log &", "cargo test --tests -- --test-threads=4 2>&1 | tee test-integration.log &", "wait"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: test-parallel",
            ),
            CorpusEntry::new(
                "M-255",
                "makefile-git-hooks",
                "Makefile installing git hooks with chmod and heredoc-like",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { phony_target("install-hooks", &[], &["echo '#!/bin/sh' > .git/hooks/pre-commit", "echo 'cargo fmt -- --check' >> .git/hooks/pre-commit", "echo 'cargo clippy -- -D warnings' >> .git/hooks/pre-commit", "chmod +x .git/hooks/pre-commit"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: install-hooks",
            ),
            CorpusEntry::new(
                "M-256",
                "makefile-conditional-os",
                "Makefile with conditional per-OS recipe",
                CorpusFormat::Makefile,
                CorpusTier::Adversarial,
                r#"fn main() { let os = "linux"; phony_target("install-deps", &[], &["if [ \"$$(uname)\" = 'Darwin' ]; then brew install openssl; elif [ \"$$(uname)\" = 'Linux' ]; then sudo apt-get install -y libssl-dev; fi"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: install-deps",
            ),
            CorpusEntry::new(
                "M-257",
                "makefile-release-checklist",
                "Makefile with multi-step release checklist recipe",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let version = "1.0.0"; phony_target("release", &["lint", "test", "build"], &["git tag -a v$(VERSION) -m 'Release $(VERSION)'", "git push origin v$(VERSION)", "cargo publish --dry-run", "echo 'Ready for cargo publish on Friday'"]); phony_target("lint", &[], &["cargo clippy -- -D warnings"]); phony_target("test", &[], &["cargo test"]); target("build", &[], &["cargo build --release"]); } fn target(n: &str, d: &[&str], r: &[&str]) {} fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                "release: lint test build",
            ),
            CorpusEntry::new(
                "M-258",
                "makefile-k8s-deploy",
                "Makefile with kubectl commands and YAML generation",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let namespace = "production"; let image = "app:latest"; phony_target("k8s-deploy", &[], &["kubectl -n $(NAMESPACE) set image deployment/app app=$(IMAGE)", "kubectl -n $(NAMESPACE) rollout status deployment/app --timeout=120s"]); phony_target("k8s-status", &[], &["kubectl -n $(NAMESPACE) get pods -o wide"]); phony_target("k8s-logs", &[], &["kubectl -n $(NAMESPACE) logs -f deployment/app --tail=100 2>&1 | head -50"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: k8s-deploy",
            ),
            CorpusEntry::new(
                "M-259",
                "makefile-terraform",
                "Makefile with terraform commands and state management",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let env = "production"; phony_target("tf-init", &[], &["terraform init -backend-config=envs/$(ENV)/backend.hcl"]); phony_target("tf-plan", &["tf-init"], &["terraform plan -var-file=envs/$(ENV)/terraform.tfvars -out=plan.tfplan"]); phony_target("tf-apply", &["tf-plan"], &["terraform apply plan.tfplan"]); phony_target("tf-destroy", &[], &["terraform destroy -var-file=envs/$(ENV)/terraform.tfvars"]); } fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                ".PHONY: tf-init",
            ),
            CorpusEntry::new(
                "M-260",
                "milestone-260-makefile",
                "Milestone 260: extreme CI/CD Makefile — SSH, K8s, terraform, heredoc, parallel",
                CorpusFormat::Makefile,
                CorpusTier::Production,
                r#"fn main() { let version = "4.0.0"; let registry = "ghcr.io/org"; let env = "production"; let ssh_host = "deploy@prod"; phony_target("ci", &["lint", "test", "build", "docker", "deploy"], &["echo 'Full CI pipeline complete'"]); phony_target("lint", &[], &["cargo clippy -- -D warnings 2>&1 | tee lint.log || exit 1"]); phony_target("test", &[], &["cargo test -- --test-threads=4 2>&1 | tee test.log || exit 1"]); target("build", &[], &["cargo build --release"]); phony_target("docker", &["build"], &["docker build -t $(REGISTRY)/app:$(VERSION) .", "docker push $(REGISTRY)/app:$(VERSION)"]); phony_target("deploy", &["docker"], &["ssh $(SSH_HOST) 'docker pull $(REGISTRY)/app:$(VERSION) && docker tag $(REGISTRY)/app:$(VERSION) $(REGISTRY)/app:latest && systemctl restart app'"]); phony_target("clean", &[], &["rm -rf target *.log"]); } fn target(n: &str, d: &[&str], r: &[&str]) {} fn phony_target(n: &str, d: &[&str], r: &[&str]) {}"#,
                "ci: lint test build docker deploy",
            ),
        ];
        self.entries.extend(entries);
    }
    ...
    // =========================================================================
    // Expansion Wave 29: Dockerfile D-231..D-240 — Round 2: EXTREME patterns
    //   SSH in builds, heredoc RUN, complex healthcheck, multi-env
    // =========================================================================

    fn load_expansion15_dockerfile(&mut self) {
        let entries = vec![
            CorpusEntry::new(
                "D-231",
                "dockerfile-ssh-build",
                "Dockerfile with SSH agent forwarding for private deps",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image_as("rust", "1.77-bookworm", "builder"); run(&["apt-get update", "apt-get install -y --no-install-recommends openssh-client git", "rm -rf /var/lib/apt/lists/*"]); workdir("/app"); env("CARGO_NET_GIT_FETCH_WITH_CLI", "true"); copy("Cargo.toml", "."); copy("Cargo.lock", "."); run(&["mkdir -p -m 0700 ~/.ssh && ssh-keyscan github.com >> ~/.ssh/known_hosts"]); copy("src/", "src/"); run(&["cargo build --release"]); from_image("debian", "bookworm-slim"); copy_from("builder", "/app/target/release/app", "/usr/local/bin/app"); entrypoint(&["/usr/local/bin/app"]); } fn from_image_as(i: &str, t: &str, a: &str) {} fn from_image(i: &str, t: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn env(k: &str, v: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn entrypoint(e: &[&str]) {}"#,
                "ENV CARGO_NET_GIT_FETCH_WITH_CLI=true",
            ),
            CorpusEntry::new(
                "D-232",
                "dockerfile-heredoc-entrypoint",
                "Dockerfile generating entrypoint script via echo/redirect chain",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image("alpine", "3.19"); run(&["apk add --no-cache bash"]); run(&["echo '#!/bin/bash' > /entrypoint.sh", "echo 'set -euo pipefail' >> /entrypoint.sh", "echo 'echo Starting app...' >> /entrypoint.sh", "echo 'exec \"$@\"' >> /entrypoint.sh", "chmod +x /entrypoint.sh"]); workdir("/app"); copy(".", "."); entrypoint(&["/entrypoint.sh"]); cmd(&["./app"]); } fn from_image(i: &str, t: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn entrypoint(e: &[&str]) {} fn cmd(c: &[&str]) {}"#,
                "FROM alpine:3.19",
            ),
            CorpusEntry::new(
                "D-233",
                "dockerfile-12-envs",
                "Dockerfile with 12 ENV variables for full app configuration",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image("python", "3.12-slim"); env("PYTHONDONTWRITEBYTECODE", "1"); env("PYTHONUNBUFFERED", "1"); env("APP_NAME", "myservice"); env("APP_PORT", "8000"); env("APP_WORKERS", "4"); env("DB_HOST", "postgres"); env("DB_PORT", "5432"); env("DB_NAME", "production"); env("REDIS_URL", "redis://cache:6379"); env("LOG_LEVEL", "info"); env("CORS_ORIGINS", "*"); env("SECRET_KEY", "changeme"); workdir("/app"); copy(".", "."); run(&["pip install --no-cache-dir -r requirements.txt"]); cmd(&["gunicorn", "app:app", "--bind", "0.0.0.0:8000"]); } fn from_image(i: &str, t: &str) {} fn env(k: &str, v: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn run(c: &[&str]) {} fn cmd(c: &[&str]) {}"#,
                "ENV APP_NAME=myservice",
            ),
            CorpusEntry::new(
                "D-234",
                "dockerfile-complex-healthcheck-pipe",
                "Dockerfile with healthcheck using pipe chain",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image("golang", "1.22-alpine"); workdir("/app"); copy(".", "."); run(&["go build -o /server"]); from_image("alpine", "3.19"); run(&["apk add --no-cache curl jq"]); copy_from("0", "/server", "/usr/local/bin/server"); expose(8080u16); healthcheck("curl -sf http://localhost:8080/health | jq -e '.status == \"ok\"' > /dev/null 2>&1 || exit 1", "10s", "3s"); cmd(&["/usr/local/bin/server"]); } fn from_image(i: &str, t: &str) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn run(c: &[&str]) {} fn expose(p: u16) {} fn healthcheck(c: &str, i: &str, t: &str) {} fn cmd(c: &[&str]) {}"#,
                "HEALTHCHECK",
            ),
            CorpusEntry::new(
                "D-235",
                "dockerfile-multi-arg",
                "Dockerfile with multiple ARG for parameterized builds",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { arg("RUST_VERSION", "1.77"); arg("ALPINE_VERSION", "3.19"); arg("APP_NAME", "myapp"); arg("FEATURES", "default"); from_image_as("rust", "$(RUST_VERSION)-alpine", "builder"); run(&["apk add --no-cache musl-dev"]); workdir("/app"); copy(".", "."); run(&["cargo build --release --features $(FEATURES)"]); from_image("alpine", "$(ALPINE_VERSION)"); copy_from("builder", "/app/target/release/$(APP_NAME)", "/usr/local/bin/app"); entrypoint(&["/usr/local/bin/app"]); } fn arg(k: &str, v: &str) {} fn from_image_as(i: &str, t: &str, a: &str) {} fn from_image(i: &str, t: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn copy_from(f: &str, s: &str, d: &str) {} fn entrypoint(e: &[&str]) {}"#,
                "ARG RUST_VERSION=1.77",
            ),
            CorpusEntry::new(
                "D-236",
                "dockerfile-cron-entrypoint",
                "Dockerfile setting up cron job with heredoc-style config",
                CorpusFormat::Dockerfile,
                CorpusTier::Adversarial,
                r#"fn main() { from_image("alpine", "3.19"); run(&["apk add --no-cache dcron"]); run(&["echo '*/5 * * * * /app/cleanup.sh > /var/log/cron.log 2>&1' > /etc/crontabs/root"]); workdir("/app"); copy(".", "."); run(&["chmod +x /app/cleanup.sh"]); cmd(&["crond", "-f", "-d", "8"]); } fn from_image(i: &str, t: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn cmd(c: &[&str]) {}"#,
                "FROM alpine:3.19",
            ),
            CorpusEntry::new(
                "D-237",
                "dockerfile-wait-for-it",
                "Dockerfile with wait-for-it pattern in entrypoint",
                CorpusFormat::Dockerfile,
                CorpusTier::Production,
                r#"fn main() { from_image("python", "3.12-slim"); run(&["apt-get update", "apt-get install -y --no-install-recommends netcat-openbsd", "rm -rf /var/lib/apt/lists/*"]); workdir("/app"); copy("requirements.txt", "."); run(&["pip install --no-cache-dir -r requirements.txt"]); copy(".", "."); run(&["echo '#!/bin/sh' > /wait-for-db.sh", "echo 'while ! nc -z $DB_HOST $DB_PORT; do sleep 1; done' >> /wait-for-db.sh", "echo 'exec \"$@\"' >> /wait-for-db.sh", "chmod +x /wait-for-db.sh"]); entrypoint(&["/wait-for-db.sh"]); cmd(&["python", "app.py"]); } fn from_image(i: &str, t: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn entrypoint(e: &[&str]) {} fn cmd(c: &[&str]) {}"#,
                "FROM python:3.12-slim",
            ),
            CorpusEntry::new(
                "D-238",
                "dockerfile-log-rotation",
                "Dockerfile with log rotation setup via redirect chains",
                CorpusFormat::Dockerfile,
                CorpusTier::Production,
                r#"fn main() { from_image("debian", "bookworm-slim"); run(&["apt-get update", "apt-get install -y --no-install-recommends logrotate", "rm -rf /var/lib/apt/lists/*"]); run(&["echo '/var/log/app/*.log {' > /etc/logrotate.d/app", "echo '  daily' >> /etc/logrotate.d/app", "echo '  rotate 7' >> /etc/logrotate.d/app", "echo '  compress' >> /etc/logrotate.d/app", "echo '  missingok' >> /etc/logrotate.d/app", "echo '}' >> /etc/logrotate.d/app"]); run(&["mkdir -p /var/log/app"]); workdir("/app"); copy(".", "."); cmd(&["./app"]); } fn from_image(i: &str, t: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn cmd(c: &[&str]) {}"#,
                "FROM debian:bookworm-slim",
            ),
            CorpusEntry::new(
                "D-239",
                "dockerfile-tls-setup",
                "Dockerfile with TLS certificate generation",
                CorpusFormat::Dockerfile,
                CorpusTier::Production,
                r#"fn main() { from_image("alpine", "3.19"); run(&["apk add --no-cache openssl"]); run(&["mkdir -p /etc/ssl/app", "openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /etc/ssl/app/key.pem -out /etc/ssl/app/cert.pem -subj '/CN=localhost'"]); workdir("/app"); copy(".", "."); env("TLS_CERT", "/etc/ssl/app/cert.pem"); env("TLS_KEY", "/etc/ssl/app/key.pem"); expose(8443u16); cmd(&["./app", "--tls"]); } fn from_image(i: &str, t: &str) {} fn run(c: &[&str]) {} fn workdir(p: &str) {} fn copy(s: &str, d: &str) {} fn env(k: &str, v: &str) {} fn expose(p: u16) {} fn cmd(c: &[&str]) {}"#,
                "ENV TLS_CERT=/etc/ssl/app/cert.pem",
            ),
            CorpusEntry::new(
                "D-240",
                "milestone-240-docker",
                "Milestone 240: extreme production — SSH, heredoc, 12 ENV, healthcheck, TLS, wait-for",
                CorpusFormat::Dockerfile,
                CorpusTier::Production,
