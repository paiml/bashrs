...
"#;
    ...
    #[cfg(unix)]
    {
        ...
        let mut perms = fs::metadata(&output_file)
            .unwrap()
            .permissions();
        perms.set_mode(0o755);
        fs::set_permissions(&output_file, perms).unwrap();

        let mut test_perms = fs::metadata(&test_file)
            .unwrap()
            .permissions();
...
"#;
    ...
    assert!(
        test_content.contains("for") || test_content.contains("while") || test_content.contains("seq"),
...
"#;
    ...
    #[cfg(unix)]
    {
        ...
        let mut script_perms = fs::metadata(&output_file)
            .unwrap()
            .permissions();
