...
impl BugReport {
    fn report(&self) {
        if self.is_bug {
            ...
            println!("   Rust code: {}", self.rust_code.lines().next().unwrap_or(""));
...
fn test_transpiler_bug_hunt_arithmetic() {
    ...
    let tests: Vec<(&str, &str, &str, Box<dyn Fn(&str) -> bool>)> = vec![
        // TB010: Addition
        ("TB010", r#"fn main() { let x = 1 + 2; }"#, "Should compute 1+2",
         Box::new(|s: &str| s.contains("$((") || s.contains("expr") || s.contains("3"))),

        // TB011: Subtraction
        ("TB011", r#"fn main() { let x = 5 - 3; }"#, "Should compute 5-3",
         Box::new(|s: &str| s.contains("$((") || s.contains("2"))),

        // TB012: Multiplication
        ("TB012", r#"fn main() { let x = 4 * 3; }"#, "Should compute 4*3",
         Box::new(|s: &str| s.contains("$((") || s.contains("12"))),

        // TB013: Division
        ("TB013", r#"fn main() { let x = 10 / 2; }"#, "Should compute 10/2",
         Box::new(|s: &str| s.contains("$((") || s.contains("5"))),

        // TB014: Modulo
        ("TB014", r#"fn main() { let x = 10 % 3; }"#, "Should compute 10%3",
         Box::new(|s: &str| s.contains("$((") || s.contains("1"))),

        // TB015: Complex expression
        ("TB015", r#"fn main() { let x = (1 + 2) * 3; }"#, "Should compute (1+2)*3",
...
fn test_transpiler_bug_hunt_control_flow() {
    ...
    for (id, rust_code, expected, check) in tests {
        ...
        if is_bug {
            println!("\n🐛 BUG: {} - {}", id, expected);
            println!("   Code: {}", rust_code.chars().take(60).collect::<String>());
            bugs_found += 1;
        }
    }
...
fn test_transpiler_bug_hunt_execution() {
    ...
    let tests = vec![
        ("TB050", r#"fn main() { println!("MARKER"); }"#, "MARKER"),
    ];
    ...
    for (id, rust_code, expected_output) in tests {
        ...
        if !success || !stdout.contains(expected_output) {
            ...
            println!("║  ✅ {}: Generated script executes correctly        PASS  ║", id);
        }
    }
