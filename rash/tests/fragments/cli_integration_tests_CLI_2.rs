...
"#;
    ...
    // The fixture is deliberately non-deterministic, so DET001 must fire and
    // the exit code must say so. Asserting `.success()` here asserted that the
    // linter MISSES $RANDOM.
    bashrs_cmd()
        .arg("lint")
        .arg(input_file.path())
        .assert()
...
"#;
    ...
    // bashrs has no Rust rules. It used to run the SHELL rules over Rust and
    // report SC1068 on `let x = 42;`; it now says what it does instead.
    bashrs_cmd()
        .arg("lint")
        .arg(input_file.path())
        .assert()
        .failure()
        .stderr(predicate::str::contains("no Rust rules"));
}
...
include!("cli_integration_tests_CLI.rs");
