...
- **The keyring tests raced on the process environment, and `keyring init` wrote nothing** (PMAT-339 + PMAT-341, #339, #341). `keyring_path()` read `$BASHRS_KEYRING` and `$HOME` from the process, and the tests set them with `std::env::set_var` — a per-PROCESS write that every other test in the binary sees. Run the module alone and it passed; run the binary's 150 tests and 61 failed, differently each time. The resolution is now a pure function of two `Option<String>` arguments (`keyring_path_from`), the caller reads the environment once (`keyring_default_path`), and the command takes the path it is given (`handle_keyring_command_at`); the 13 tests exercise the resolution directly and not one of them writes the process environment. Measured: 61/150 failures before, 0/150 after. The second defect was in the same file and invisible behind the first — `keyring init` built a keyring and dropped it, so the file it reported creating did not exist; it calls `keyring.save()?` now, and a test reads the file back.

- **`corpus-score`'s sandbox masked the repository it was told to run from** (PMAT-340, #340). `make release-gate` runs the corpus through `bwrap` with `--tmpfs /tmp` over the whole of `/tmp`, and then executes `$(PWD)/target/debug/bashrs`. For a checkout under `/tmp` — every worktree this fleet builds in — that binary is behind the tmpfs: `bwrap: execvp <worktree>/target/debug/bashrs: No such file or directory`, and the release gate could not start at all. `$(PWD)` is re-exposed READ-ONLY over the tmpfs, which is exactly what `--ro-bind / /` already granted everywhere outside `/tmp` and nothing more. The read-write form was tried first and works, so the weaker mount is the right one: both score the corpus, 18,814 entries, rc=0. Writes belong in the scratch directory `--chdir` already selects.

- **`bashrs fix` rewrote working scripts into different programs** (PMAT-335, #335). SC2081 and SC2016 found single-quoted strings with a line regex that tracked no quote state, so in `assert_row 'append one entry' PASS "$TD/append.yaml" 'added=1'` a match ran from the closing quote of the first string to the opening quote of the last, and SC2081's fix, marked safe and therefore applied by plain `bashrs fix`, wrapped that span in double quotes: five arguments became two. On `trap 'rm -rf -- "${TD:?}"' EXIT` the same fix wrote `trap "rm -rf -- "${TD:?}"" EXIT`, which expands `$TD` when the trap is set instead of when it fires. `bash -n` accepts both. The two rules now find strings through the shell-word lexer (`linter::quoted_segments`), so a match never crosses a string boundary. SC2081's fix is safe-with-assumptions, applied only under `--assumptions`, because turning `'…$x…'` into `"…$x…"` changes what runs, and it escapes embedded `"`, `\` and backticks. And `apply_fixes` refuses any fix, from any rule, that merges two words of a top-level command, adds or removes a command, or adds an unquoted expansion to the line; a fix may still add a word, as the idempotency fixes' `mkdir -p` does. That net found the same defect in the suite: `test_fix_priority_sc2046_coverage` hand-wrote a span three bytes past `$(cat file.txt)`, so its fix produced `cp "$(cat file.txt)"est`, and its `contains` assertion passed; it now uses the span the rule emits and asserts the whole output. `rash/tests/cli_fix_335.rs` runs the issue's script under bash before and after `bashrs fix` and requires the same output.
...
- 18,592 to 18,814 entries, 222 in all: 203 from the first validation pass and 19 more unlocked by PMAT-265 (below). Six blind generation lanes wrote 240 candidates; each was transpiled through the library and its expected line matched against the output (216 passed), every Bash survivor was run under bwrap with a two-second timeout (143 ran, none timed out), and 13 duplicated an existing name. The 203 added in the first pass cover cron-field parsing and validation in shell text, Makefile recipes, retry loops, env defaults and text pipelines: 143 Bash, 60 Makefile; 77 Adversarial, 63 Production, 54 Standard, 6 Complex, 3 Trivial.
...
- 19 of the 24 candidates that failed the first validation pass now transpile after PMAT-265 and were added (ids 17704..17722): env defaults with braces, quotes, dollars and a literal `$(pwd)` in the default, and arithmetic inside `capture()`. The five still out are four spellings of `.len()` **chained directly onto** the defaulted read (`std::env::var("X").unwrap_or(d).len()`, which has no variable to take `${#var}` on) and one `push_str`, both honest transpile errors naming the method. Binding the read to a variable first does work and is covered: B-17719, B-17720 and B-17721 take `.len()` on such a variable, which lowers to `${#v}`.
...
Seven of the thirteen issues left open after 7.0.3, all `release:7.0.4` after a
measured triage (PMAT-252), plus the dependency refresh and a corpus that now
covers the shell we actually write: crontabs, Makefile operations, perl and
python inside bash, and agentic tooling pipelines.

### Fixed

- **MAKE010** matched command names inside quoted strings and subcommand
  tokens (`cargo install` → `install`), and asked for `|| exit 1` where make
  already stops on failure; it also flagged `rm -f` / `mkdir -p`, the flags
  that *are* the error handling. The rule now reads the command word of each
  simple command and honours tolerant flags; `cp b a` inside an `if` body is
  still reported (#256, #257).
- **SEC011** did not recognise the `[ -n "$X" ] || exit 1` early-return guard
  although the `if` and `&&` forms were recognised (#264).
- **DET002** missed a bare `date > VERSION` while reporting
  `echo "$(date)" > VERSION`; a bare `date` whose stdout reaches a sink is
  detected, `date` to the terminal still is not (#263).
- **SC2188** never examined a line starting with a file-descriptor digit, so
  a lone `2>&1` was not reported (#249).
- **SC1009/BRS0001** reported a comment leading a non-empty block as an empty
  body (#238).
- Seven git-tracked `.rs` files that nothing compiled (file-splitter residue)
  are deleted; zero references measured (#254).
- CI test-job headroom (#287) closed with evidence: 13m37s against the
  60-minute limit on run 34459917312 after PMAT-247, down from 53m46s.

### Changed

- **Dependencies** refreshed to the latest versions that build (PMAT-251):
  194 compatible lockfile updates, and toml 1.1, rand/rand_chacha 0.10
  (`Rng` → `RngExt`), schemars 1.2, notify 8.2 + notify-debouncer-mini 0.7,
  zstd 0.14, base64 0.23, ratatui 0.30 + crossterm 0.29, rustyline 18, phf 0.14,
  hashbrown 0.17, itertools 0.15, criterion 0.8, rstest 0.27, assert_cmd 2.2,
  verificar 0.6, provable-contracts 0.4, safetensors 0.8, entrenar 0.8, syn 3.0.
  Held below latest, each with the measured refusal in
  `docs/audits/deps-exceptions-PMAT-251.txt`: sysinfo 0.38 (0.39 needs rustc
  1.95; the toolchain pins 1.93), renacer 0.9 (0.10/0.11 conflict on
  unicode-width with rustyline 18), aprender 0.27/0.26 (0.66 dropped the
  `parallel` and `format-compression` features). `bashrs-oracle` is now the in-tree crate
  (`path = "../bashrs-oracle"`, pmat comply CB-081-F); the stale
  `regex-automata 0.5` / `regex-syntax 0.9` unification pins are gone.
- **Corpus**: +236 entries (18,178 total) generated by six agy lanes under a
  dedicated watcher and measured entry by entry — crontab install/list guards,
  Makefile release/ci/comply/worktree targets, `python3 -c` / `perl -pe` /
  `awk` inside bash and recipes, cron inside containers, gh/pmat/cargo
  pipelines. 237 predicted output lines matched on the first run, 0 were
  pinned afterwards; 3 candidates are held in
  `docs/audits/corpus-pending-PMAT-251.jsonl` with their measured reason
  (one malformed input, one unsafe exec string, one transpiler defect filed
  as #294). The growth also surfaced #293 (`array_join` over a local array
  emits an unset `$items`) and #295 (`cargo build --workspace --all-targets`
  fails on bashrs-wasm's probar test target, pre-existing).
- **Compliance**: the `adversarial-crux-pr` skill carries an `effort:` pin
  (CB-1650); `.github/required-status-checks.txt` declares `gate` for
  CB-1701/CB-2100.
- **Docs**: CLAUDE.md now states the measured unwrap posture (`unwrap_used`
  is `allow`, enforced only by `make lint-check`; `deny` is the target).

### Measured

```text
bashrs corpus run (7.0.4 release binary) — 18,178 entries, V2 Corpus Score 84.4/100 (B)
  ...
  C  Coverage        0.0%   0.0/15   <- no coverage cache for this commit (same as 7.0.3)
```

D (lint clean) is 18,176/18,178 (7.0.3: 17,940/17,942 — the two pre-existing exceptions are unchanged; all 236 new entries are lint-clean). Per format: bash 16,513/16,513, makefile 900/900, dockerfile 765/765 passed. `cargo test --workspace --lib`: 15288 passed, 0 failed. `pmat comply check` on the release branch: 166 checks, 7 fail (7.0.3: 10) — CB-081-F, CB-1351, CB-1650 and Version Currency cleared; CB-400 unchanged at 1,253 errors / 5,584 warnings; CB-200 771 functions below A (was 777); CB-1305/CB-1308/CB-2100 and the two classic-API branch-protection checks remain and are recorded in `.pmat.yaml` and the PMAT-251 receipt.
...
- `bashrs explain` CLI command — natural-language safety explanation (SSC v11 S8.1)
  ...
  - Per-finding what/why/fix explanations for SEC, DET, IDEM rules
...
- `bashrs corpus publish-dataset` CLI command — generate complete HuggingFace-ready dataset directory (SSC v11 S9)
