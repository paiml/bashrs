...
- **Dependencies** refreshed to the latest versions that build (PMAT-251):
  ...
  Kept, each with the measured refusal in
  `docs/audits/deps-exceptions-PMAT-251.txt`: sysinfo 0.31 (0.39 needs rustc
  1.95; the toolchain pins 1.93), renacer 0.7 (unicode-width conflict with
  rustyline 18), aprender 0.27/0.26 (0.66 dropped the `parallel` and
  `format-compression` features). `bashrs-oracle` is now the in-tree crate
