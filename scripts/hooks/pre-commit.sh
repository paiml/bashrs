...
# Generated at: 2026-02-07 15:29:52
...
if [ -n "$STAGED_RS" ]; then
    ...
    COMPLEXITY_FAILED=0
    COMPLEXITY_DETAILS=""
    for RS_FILE in $STAGED_RS; do # comply:disable=COMPLY-005
        if [ -f "$RS_FILE" ]; then
...
# 5. Corpus unit tests — fast gate (spec §7.4, <30s)
# Only run when corpus/transpiler/emitter files are staged
CORPUS_FILES=$(echo "$STAGED_RS" | grep -E 'corpus|transpil|emit|ir/' || true)
if [ -n "$CORPUS_FILES" ]; then
    echo -n "  Corpus tests... "
    if cargo test -p bashrs --lib -- corpus --quiet 2>/dev/null; then
        echo "✅"
    else
        echo "❌"
        echo "   Corpus unit tests failed — STOP THE LINE (spec §5.3)"
        exit 1
    fi
elif [ -n "$STAGED_RS" ]; then
    echo "  Corpus tests... ⏭️  (no corpus-related files staged)"
fi
